...
def write_candidate_csv(
    path: Path, rows: list[tuple[str, float, float]]
) -> None:
    ...
    with path.open("w", encoding="utf-8", newline="") as handle:
        ...
        for entity, embedding_score, character_score in rows:
            writer.writerow(
                (entity, f"{embedding_score:.4f}", f"{character_score:.4f}")
            )
...
def exact_top_rows(
    ...
    selected = np.argpartition(hybrid_scores, -FREQUENT_CANDIDATES)[
        -FREQUENT_CANDIDATES:
    ]
    ordered = sorted(
...
def build(
    ...
    try:
        ...
        with zipfile.ZipFile(archive_path) as archive:
            ...
            vector_root = tempfile.TemporaryDirectory(
                prefix="konbaung_frequency_vectors_"
            )
        ...
        frequent_indices = [
            ...
            if frequency > expanded_threshold
        ]
        ...
        for batch_start in range(0, len(frequent_indices), FREQUENT_BATCH_SIZE):
            batch_indices = frequent_indices[
                batch_start : batch_start + FREQUENT_BATCH_SIZE
            ]
            ...
            for local_index, entity_index in enumerate(batch_indices):
                ...
                write_candidate_csv(
                    entities_output / filenames[entity_index], rows
                )
            print(
        ...
        for index, filename in enumerate(filenames):
            with (entities_output / filename).open(
                encoding="utf-8", newline=""
            ) as handle:
