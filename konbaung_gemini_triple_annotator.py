#!/usr/bin/env python3
"""Gemini triple annotator for Konbaung per-page OCR files.

This is an alternate, simpler pipeline: each output item is one fused
subject-predicate-object triple with subject/object labels and an English gloss.
It does not emit separate node records, relationship records, or properties.
"""
...
DEFAULT_OUT_DIR = Path("konbaung_triple_annotations_ready")
DEFAULT_MAX_OUTPUT_TOKENS = 8000


class Mention(BaseModel):
    ln: int = Field(description="1-based line number in <page_text>.")
    tx: str = Field(description="Exact original evidence substring from that line.")
...
class TripleOut(BaseModel):
    s: str = Field(description="Exact Burmese subject text from the page.")
    ...
    o: str = Field(description="Exact Burmese object text from the page.")
    ol: list[EntityType] = Field(description="One or more existing entity labels for the object.")
    m: list[Mention] = Field(description="Minimal exact evidence mention(s) for this specific triple.")
...
Return JSON with top-level key T. Each item in T requires: s, sl, p, o, ol, m, en.
s and o are exact Burmese text from the page. sl and ol use only the entity labels below. p uses only the relation types below. m gives the shortest exact evidence text for the claim: ln, tx, i. en is the English gloss.
...
def resolve_mention(lines: list[str], mention: Mention) -> tuple[Optional[dict[str, Any]], Optional[str]]:
...
def resolve_annotation(job: PageJob, annotation: TripleAnnotation) -> tuple[dict[str, Any], list[str]]:
    ...
    for idx, triple in enumerate(annotation.T, start=1):
        ...
        if not triple.m:
            errors.append(f"{triple_id}: no evidence mentions")
        ...
        triple_data["m"] = resolved_mentions
...
def build_marked_page_text(job: PageJob, annotation: dict[str, Any], validation_errors: list[str]) -> str:
    ...
    for triple_idx, triple in enumerate(annotation.get("T", []), start=1):
        for mention_idx, mention in enumerate(triple.get("m", []), start=1):
            spans = mention.get("spans") or []
            if not spans and {"ln", "s", "e"}.issubset(mention):
                spans = [{"ln": mention["ln"], "s": mention["s"], "e": mention["e"]}]
...
def prepare_outputs(args: argparse.Namespace, jobs: list[PageJob]) -> None:
    ...
    summary = {
        ...
        "schema": "fused triples: T[].s/sl/p/o/ol/m/en",
        "model": args.model,
...
def parse_args(argv: Optional[list[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Prepare or run Gemini fused-triple annotation over Konbaung pages.")
    parser.add_argument("--source-root", default=str(DEFAULT_SOURCE_ROOT), help="Root containing konbaung_vol*/pages/page_*.txt.")
    parser.add_argument("--out-dir", default=str(DEFAULT_OUT_DIR), help="Output directory for triple annotations.")
