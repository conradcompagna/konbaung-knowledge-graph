...
ARTIFACT_DIR = Path(__file__).resolve().parents[2] / "konbaung_reader_app" / "test-artifacts"
...
def main() -> None:
    ...
    with sync_playwright() as playwright:
        ...
        try:
            ...
            page.locator('[data-triple-variant="v3"][aria-pressed="true"]').wait_for()
            page.wait_for_function(
                "() => document.querySelectorAll('#triple-list [data-triple-id]').length === 16"
            )
            ...
            page.locator("#inspector-pane").evaluate("element => { element.scrollTop = 0; }")
            page.locator('#canonical-text [data-sentence-id="vol1_s000003"]').first.click()
            ...
            if (
                ...
                raise AssertionError(f"V3 sentence did not navigate to its group: {navigation}")
            if (
                navigation["relationPath"]
                or navigation["textMemberships"]
                or navigation["sentenceHighlights"] <= 0
            ):
                raise AssertionError(f"V3 sentence navigation highlight is wrong: {navigation}")
            ...
            page.screenshot(
                path=str(ARTIFACT_DIR / "reader-v3-ungrounded-sentence-navigation.png"),
                full_page=True,
            )

            page.locator('[data-triple-variant="v2"]').click()
            page.locator('[data-triple-variant="v2"][aria-pressed="true"]').wait_for()
            page.wait_for_function(
                "() => document.querySelectorAll('#triple-list [data-triple-id]').length === 9"
            )
            if page.locator("#annotation-toggle").is_disabled():
