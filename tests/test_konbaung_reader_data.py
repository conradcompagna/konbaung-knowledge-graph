...
from build_konbaung_reader_data import span_candidates, utf16_offset
from build_konbaung_sentence_reader_data import (
...
class AlignmentUnitTests(unittest.TestCase):
    ...
    def test_structured_token_glosses_drop_inserted_extras(self) -> None:
        ...
        validated = validate_token_glosses(glosses, tokens)
        self.assertEqual([(item["my"], item["en"]) for item in validated], [("က", "ka"), ("ခ", "kha")])
...
class BuiltCorpusTests(unittest.TestCase):
        ...
        for volume in self.index["volumes"]:
            for page_number in volume["availablePages"]:
                ...
                for annotation in page["annotations"]:
                    self.assertEqual(annotation["provenance"]["pass"], "translated_sentence_triples")
    ...
    def test_canonical_source_checksum_and_text(self) -> None:
        ...
        self.assertEqual(page["source"]["checksum"], hashlib.sha1(exact.encode("utf-8")).hexdigest())
    ...
    def test_every_exact_endpoint_round_trips(self) -> None:
        ...
        for volume in self.index["volumes"]:
            for page_number in volume["availablePages"]:
                ...
                for annotation in page["annotations"]:
                    for role in ("subject", "predicate", "object"):
                        endpoint = annotation["relation"] if role == "predicate" else annotation[role]
    ...
    def test_v3_is_sentence_linked_but_has_no_page_spans(self) -> None:
        ...
        for volume in self.index["volumes"]:
            for page_number in volume["availablePages"]:
                ...
                annotations = {
                    annotation["id"]: annotation for annotation in page["annotations"]
                }
                assigned = [
                    triple_id
                    for sentence in page["sentences"]
                ...
                unavailable_sentence_count += page["diagnostics"][
                    "unavailableSentenceCount"
                ]
    ...
    def test_v3_duplicate_selection_prefers_most_triples(self) -> None:
        ...
        for volume in self.index["volumes"]:
            for sentence in self.store._v3_sentences(volume["id"]).values():
                ...
                top = [
                    item for item in appearances if item["tripleCount"] == maximum
                ]
                owner_page = sentence["ownerPage"]
                expected_page = min(
                    (item["page"] != owner_page, item["page"]) for item in top
                )[1]
    ...
    def test_every_v1_triple_maps_once_to_a_current_translated_sentence(self) -> None:
        ...
        for volume in self.index["volumes"]:
            for page_number in volume["availablePages"]:
                ...
                annotations = {
                    annotation["id"]: annotation for annotation in v1["annotations"]
                }
                assigned = [
                    triple_id
                    for sentence in v1["sentences"]
                ...
                self.assertEqual(set(assigned), set(annotations))
                self.assertTrue(
                    all(annotation["evidence"]["status"] == "resolved" for annotation in annotations.values())
                )
                annotation_count += len(annotations)
