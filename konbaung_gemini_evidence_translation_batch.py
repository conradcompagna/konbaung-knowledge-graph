...
from konbaung_gemini_evidence_translation_test import (
    TRANSLATION_PROMPT,
    build_payload,
    validate,
    write_review,
...
class TranslationResult(BaseModel):
    translations: list[TranslationItem] = Field(description="One ordered translation per evidence item.")
...
def submit_batch(
    ...
    batch = client.batches.create(
        model=model,
        src=requests,
        config=types.CreateBatchJobConfig(display_name=f"konbaung_translation_{label}_{utc_stamp()}"),
    )
...
def process_batch(out_dir: Path, label: str, batch: Any, jobs: list[TranslationJob]) -> dict[str, Any]:
...
def run(args: argparse.Namespace) -> None:
    ...
    print(json.dumps({"complete": True, "volumes": [item["totals"] for item in summaries]}, indent=2))
