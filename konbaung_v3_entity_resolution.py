...
def build_fasttext_tag_vectors(records: list[dict]) -> tuple[np.ndarray, list[dict]]:
    ...
    if path.exists() and report_path.exists():
        return np.load(path, mmap_mode="r"), json.loads(
    ...
    output = np.lib.format.open_memmap(
        path, mode="w+", dtype=np.float32, shape=(len(records), 300)
    )
    ...
    for record in records:
        ...
        for token in token_row["coveredTokens"]:
            ...
            vector_sum += (
                np.asarray(
                    language_vectors[language][vector_index], dtype=np.float32
                )
                * weight
            )
            weight_sum += weight
...
def build_relation_vectors(
    ...
    output = np.lib.format.open_memmap(
        path, mode="w+", dtype=np.float32, shape=(len(records), 768)
    )
...
def pair_features(
    ...
    for start in range(0, len(pairs), 50000):
        ...
        fasttext_similarity = np.einsum(
            "ij,ij->i", fasttext[left], fasttext[right]
        )
        fasttext_available = (
            (np.linalg.norm(fasttext[left], axis=1) > 0)
        ...
        output[start : start + len(batch), 4] = fasttext_available.astype(
            np.float32
        )
    return output
...
def all_pair_similarity(
    ...
    for position, (matrix, weight) in enumerate(zip(matrices, weights)):
        part = np.asarray(matrix, dtype=np.float32) @ np.asarray(
...
def run_alias_resolution() -> None:
    ...
    for record in records:
        lexical_groups[normalized_display(record["tag"])].append(
            int(record["index"])
        )
    ...
    if pair_cache.exists() and feature_cache.exists():
        ...
        for start in range(0, len(pairs), 50000):
            ...
            left = batch[:, 0]
            right = batch[:, 1]
            features[start : start + len(batch), 0] = np.einsum(
                "ij,ij->i", base[left], base[right]
            )
    ...
    for pair_index, (left, right) in enumerate(pairs):
        left = int(left)
        right = int(right)
        format_equivalent[pair_index] = (
            normalized_display(records[left]["tag"])
    ...
    similarity_floor = max(
        0.972,
        float(np.quantile(composite[auto_mask], 0.05))
        if np.any(auto_mask)
        else 0.975,
    )
    ...
    for component in nx.connected_components(graph):
        ...
        if len(members) <= 1:
            ...
            continue
        indices = np.asarray(members, dtype=np.int64)
        base_similarities = np.asarray(base[indices], dtype=np.float32) @ np.asarray(
            base[indices], dtype=np.float32
        ).T
        context_similarities = np.asarray(
            context[indices], dtype=np.float32
        ) @ np.asarray(context[indices], dtype=np.float32).T
        similarities = 0.20 * base_similarities + 0.80 * context_similarities
        similarities[
            np.maximum(base_similarities, context_similarities) < 0.98
        ] = 0.0
    ...
    for cluster_number, members in enumerate(final_groups, start=1):
        ...
        if len(members) > 1:
            ...
            np.fill_diagonal(similarities, 1.0)
            confidence = float(
                similarities[np.triu_indices(len(members), k=1)].min()
            )
        else:
            confidence = 1.0
        variants = sorted(
    ...
    with (ALIAS_ROOT / "alias_assignments.jsonl").open(
        "w", encoding="utf-8"
    ) as handle:
    ...
    review_pairs.sort(
        key=lambda row: (-row["probability"], -row["compositeSimilarity"])
    )
    ...
    diagnostics = {
        name: assignments[tag_to_index[name]]
        for name in diagnostic_names
        if name in tag_to_index
    }
    report = {
        "method": (
        ...
        "cooccurringListPairsRejected": int(
            np.sum(cooccurring_list_pair & ~format_equivalent)
        ),
        "completeLinkSimilarityFloor": similarity_floor,
        "connectedComponentsSplit": split_components,
        "clusters": len(clusters),
        "multiVariantClusters": sum(
            cluster["variantCount"] > 1 for cluster in clusters
        ),
        "tagsInMultiVariantClusters": sum(
            cluster["variantCount"]
...
class VectorStore:
    def __init__(self, name: str) -> None:
        ...
        self.key_to_row = {
            row["key"]: int(row["row"]) for row in jsonl(key_path)
        }
...
def build_mentions(
    ...
    with record_path.open("w", encoding="utf-8") as handle:
        for occurrence_position, occurrence in enumerate(occurrences):
            triple_vector = triple_context.combine(
                occurrence["embeddingKeys"]["SPOBE"]
            )
            endpoint_specs = (
                (
                    occurrence["subject"],
            ...
            for endpoint_ordinal, (tag, counterpart, context_keys) in enumerate(
                endpoint_specs
            ):
    ...
    return records, np.load(residual_path, mmap_mode="r"), np.load(
        triple_path, mmap_mode="r"
    )
...
def build_alias_statistics(
    ...
    for cluster in alias_clusters:
        ...
        for variant in cluster["variants"]:
            ...
            base_sums[cluster_index] += (
                np.asarray(base[tag_to_index[variant["tag"]]], dtype=np.float32)
                * frequency
            )
    ...
    triple_sums = np.asarray(
        membership @ np.asarray(triples, dtype=np.float32), dtype=np.float32
    )
    ...
    triple_cohesion = np.linalg.norm(
        triple_sums / np.maximum(mention_counts[:, None], 1), axis=1
    )
    ...
    statistics = []
    for index, cluster in enumerate(alias_clusters):
        cohesion = 0.75 * float(residual_cohesion[index]) + 0.25 * float(
            triple_cohesion[index]
        )
        ambiguity_score = (
        ...
        statistics.append(
            {
                ...
                "residualContextCohesion": round(
                    float(residual_cohesion[index]), 6
                ),
...
def run_coreference() -> None:
    ...
    for row in statistics:
        ...
        seed_similarity = float(
            node_context[seed_tag_index] @ node_context[candidate_tag_index]
        )
        if (
            row["aliasIndex"] == seed_alias_index
            or seed_similarity >= 0.975
        ):
    ...
    for row in mention_records:
        ...
        page_mentions[(row["volumeId"], int(row["ownerPage"]))].append(
            mention_index
        )
    ...
    for identity, seeds in RULER_IDENTITY_SEEDS.items():
        ...
        for alias in identity_aliases:
            if alias in ruler_alias_to_identity:
                raise RuntimeError(
                    f"Alias cluster {alias} assigned to multiple ruler identities"
                )
    ...
    for alias in ruler_alias_to_identity:
        for mention_index in mention_indices_by_alias[alias]:
            ...
            ruler_page_mentions[
                (mention["volumeId"], int(mention["ownerPage"]))
            ].append(mention_index)
    ...
    for selected_row in selected:
        ...
        indices = np.asarray(
            mention_indices_by_alias[current_alias], dtype=np.int64
        )
        ...
        for mention_index in mention_indices_by_alias[current_alias]:
            ...
            for candidate_page in range(page - 8, page + 9):
                candidate_mentions.extend(
                    ruler_page_mentions.get(
                        (mention["volumeId"], candidate_page), []
                    )
                )
            ...
            source_residual = np.asarray(
                residuals[mention_index], dtype=np.float32
            )
            ...
            for identity, candidate_indices in grouped_candidates.items():
                ...
                contextual = (
                    0.75
                    * (
                        np.asarray(residuals[candidate_array], dtype=np.float32)
                        @ source_residual
                    )
                    + 0.25
                    * (
                        np.asarray(triples[candidate_array], dtype=np.float32)
                        @ source_triple
                    )
                )
                ...
                distances = np.abs(
                    np.asarray(
                        [
                            int(mention_records[index]["ownerPage"])
                            for index in candidate_array
                        ],
                        dtype=np.float32,
                    )
                    - page
                ...
                tag_similarity = float(
                    alias_base[current_alias]
                    @ ruler_identities[identity]["baseVector"]
                )
                score = (
                    0.65 * best_context
            ...
            resolution_rows.append(
                {
                    ...
                    "status": status,
                    "resolvedAliasClusterId": (
                        None
                    ),
                    "resolvedCanonicalTag": (
                        best[1] if status == "resolved" else None
                    ),
                    ...
                    "contextSimilarity": (
                        round(float(best[2]), 6) if best else None
                    ),
                    "tagEmbeddingSimilarity": (
                        round(float(best[3]), 6) if best else None
                    ),
                    "localAnchorSupport": (
                        round(float(best[4]), 6) if best else None
                    ),
                    "supportMentionId": (
                        mention_records[best[5]]["mentionId"] if best else None
                    ),
                    "supportTag": (
                        mention_records[best[5]]["tag"] if best else None
                    ),
                    "supportPage": (
                        int(mention_records[best[5]]["ownerPage"])
                        if best
                        else None
                    ),
                }
    ...
    with (COREF_ROOT / "coreference_resolutions.jsonl").open(
        "w", encoding="utf-8"
    ) as handle:
    ...
    write_json(
        COREF_ROOT / "ruler_identity_anchors.json",
        {
            identity: {
                ...
                "aliasClusterIds": [
                    alias_clusters[index]["aliasClusterId"]
                    for index in row["aliasIndices"]
                ],
    ...
    report = {
        ...
        "resolutionRate": accepted / len(resolution_rows)
        if resolution_rows
        else 0.0,
        "sourceFilesModified": False,
    }
...
def main() -> None:
    ...
    parser.add_argument(
        "--stage", choices=("alias", "coref", "all"), default="all"
    )
