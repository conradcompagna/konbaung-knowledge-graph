...
def supplied_page(record: dict[str, Any]) -> dict[str, Any]:
    decisions = {
        sentence["sid"]: sentence for sentence in record["response"]["sentences"]
    }
    sentences = []
...
def validate(
    ...
    if any(
        mapping.category_id not in set(entity_category_ids)
    ...
    if previous is not None:
        ...
        for category in previous.entity_categories:
            if current_entity_categories.get(category.id) != category.label:
                errors.append(
                    f"Previous entity category was not preserved: "
...
def reconcile_carried_mappings(
    ...
    current_entities = {
        mapping.entity: mapping.category_id for mapping in result.entity_mappings
    }
    current_relations = {
    ...
    if len(current_entities) == len(result.entity_mappings) and not (
        ...
        for entity in entities:
            ...
            reconciled_entities.append(
                EntityMapping(entity=entity, category_id=category_id)
            )
    ...
    if len(current_relations) == len(result.relation_mappings) and not (
        ...
        for relation in relations:
            ...
            if relation in previous_relations:
                ...
                if category_id is None:
                    repairs.append(
                        f"Restored recurring relation `{relation}` as "
                        f"`{stable_category_id}`"
                    )
                elif category_id != stable_category_id:
                    repairs.append(
            ...
            reconciled_relations.append(
                RelationMapping(relation=relation, category_id=category_id)
            )
...
def review_markdown(
    ...
    new_entity_categories = [
        ...
        if category.id not in previous_entity_ids
    ]
    ...
    carried_entity_categories = [
        category
        for category in result.entity_categories
        if category.id in previous_entity_ids
    ]
    carried_relation_categories = [
        category
        for category in result.relation_categories
        if category.id in previous_relation_ids
    ]
    ...
    for category in result.entity_categories:
        ...
        lines.extend(
            [f"### `{category.id}` — {category.label}{new_marker}", ""]
        )
...
def merge_codebooks(
    ...
    if previous is not None:
        entity_mappings.update(
            {mapping.entity: mapping for mapping in previous.entity_mappings}
        )
...
def load_previous_result(path: Path) -> CategorizationResult:
    ...
    if codebook_path.exists():
        return CategorizationResult.model_validate_json(
