...
from build_konbaung_reader_data import (
...
DEFAULT_OUTPUT_ROOT = WORKSPACE / "konbaung_reader_app" / "static" / "data" / "konbaung_sentence_staging"
...
def exact_compact_candidates(
    ...
    while True:
        ...
        for index in range(1, len(mapped) + 1):
            if index < len(mapped) and mapped[index]["pieceIndex"] == mapped[group_start]["pieceIndex"]:
                continue
...
def resolve_cross_page_triple(
    ...
    if unresolved_roles and combination_count <= 4096:
        scored_combinations: list[
            tuple[tuple[int, int, float], tuple[dict[str, Any], ...]]
        ] = []
...
def build(args: argparse.Namespace) -> dict[str, Any]:
    ...
    page_sources: dict[tuple[int, int], list[tuple[dict[str, Any], dict[str, Any]]]] = defaultdict(list)
    ...
    expected_pages = {(int(read_json(path)["volume"]), int(read_json(path)["page"])) for path in page_record_paths}
        ...
        for sentence, source_pieces in ordered_sources:
            ...
            for source_piece in source_pieces:
                ...
                if canonical_text[start_cp:end_cp] != source_text:
                    raise ValueError(f"Source offset mismatch: {volume_id} page {page_number}, {sentence_id}")
                if utf16_offset(canonical_text, start_cp) != int(source_piece["start_utf16"]):
                    raise ValueError(f"UTF-16 start mismatch: {volume_id} page {page_number}, {sentence_id}")
                if utf16_offset(canonical_text, end_cp) != int(source_piece["end_utf16"]):
                    raise ValueError(f"UTF-16 end mismatch: {volume_id} page {page_number}, {sentence_id}")
            ...
            for triple_index, triple in enumerate(sentence_triples, start=1):
                ...
                if sentence["cross_page"]:
                    ...
                    if not any(
                        endpoint_is_present(endpoint)
                        for endpoint in (subject, predicate, object_)
                    ):
                ...
                warnings = [f"subject_{item}" for item in subject_warnings] + [
