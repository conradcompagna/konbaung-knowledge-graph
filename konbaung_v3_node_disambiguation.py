...
def repair_utf8_mojibake(value: Any) -> Any:
    ...
    try:
        raw = bytes(
            ord(char) if ord(char) <= 255 else CP1252_REVERSE[char]
            for char in value
        )
...
def xlsx_table(path: Path, sheet_name: str) -> list[dict[str, Any]]:
    ...
    with ZipFile(path) as archive:
        ...
        if "xl/sharedStrings.xml" in archive.namelist():
            ...
            shared_strings = [
                "".join(
                    text.text or ""
                    for text in item.iter(f"{main}t")
                )
        ...
        sheet = next(
            (
                row
                for row in sheets
                if row.attrib.get("name") == sheet_name
            ),
            None,
        )
        if sheet is None:
    ...
    for row in sheet_root.findall(f".//{main}sheetData/{main}row"):
        ...
        for cell in row.findall(f"{main}c"):
            ...
            if cell_type == "s" and value_element is not None:
                ...
                value = "".join(
                    text.text or ""
                    for text in inline_element.iter(f"{main}t")
                )
    ...
    return [
        {
            str(headers[index]): row.get(index)
            for index in range(header_width)
        }
...
def load_expanded_manual_seeds(
    ...
    for tag, row in manual_by_tag.items():
        ...
        if existing is not None and existing["canonicalLabel"] != canonical:
            raise RuntimeError(
                f"Conflicting canonical labels for manual seed {seed_id}"
            )
        seed_meta[seed_id] = {
    ...
    for row in rows:
        ...
        if int(row["member_count"]) != observed_member_counts[seed_id]:
            raise RuntimeError(
                f"Manual member-count mismatch for seed {seed_id}"
            )
        if (
            int(row["cluster_total_frequency"])
            != observed_frequency_totals[seed_id]
        ):
            raise RuntimeError(
                f"Manual frequency-total mismatch for seed {seed_id}"
            )
    ...
    audit = {
        ...
        "singletonManualClusters": sum(
            len(members) == 1 for members in manual_members.values()
        ),
        "multiTagManualClusters": sum(
            len(members) > 1 for members in manual_members.values()
        ),
        "utf8MojibakeTagRepairs": repair_log,
        "actions": dict(Counter(str(row["action"]) for row in rows)),
        "confidence": dict(
            Counter(str(row["confidence"]) for row in rows)
        ),
        "qualityFlags": dict(
            Counter(
                str(row["quality_flag"] or "")
                for row in rows
            )
        ),
    }
...
class PairFeatures:
    ...
    def matrix(self, pairs: list[tuple[int, int]]) -> np.ndarray:
        ...
        matrix = np.empty(
            (len(pairs), len(FEATURE_NAMES)), dtype=np.float32
        )
        matrix[:, 0] = np.einsum(
            "ij,ij->i",
        ...
        matrix[:, 3] = np.asarray(
            self.word_matrix[left]
            .multiply(self.word_matrix[right])
            .sum(axis=1)
        ).reshape(-1)
        matrix[:, 4] = np.asarray(
            self.char_matrix[left]
            .multiply(self.char_matrix[right])
            .sum(axis=1)
        ).reshape(-1)
        ...
        for row, (left_index, right_index) in enumerate(pairs):
            ...
            shared_weight = sum(
                self.token_idf.get(token, 0.0) for token in shared
            )
            left_weight = sum(
                self.token_idf.get(token, 0.0) for token in left_set
            )
            right_weight = sum(
                self.token_idf.get(token, 0.0) for token in right_set
            )
            denominator = min(left_weight, right_weight)
            matrix[row, 6] = (
                shared_weight / denominator if denominator > 0 else 0.0
            )
            ...
            matrix[row, 9] = float(
                self.normalized_text[left_index]
                == self.normalized_text[right_index]
            )
            matrix[row, 10] = float(
                bool(left_set)
                and bool(right_set)
            ...
            matrix[row, 11] = float(
                self.numeric[left_index] == self.numeric[right_index]
            )
            matrix[row, 12] = float(
                bool(self.numeric[left_index])
                and bool(self.numeric[right_index])
            )
            matrix[row, 13] = float(
...
def strict_conjunction_reordering(
    ...
    for left_token, right_token in zip(
        reversed(left_second), reversed(right_second)
    ):
...
def learn_safe_role_tokens(
    ...
    for pair, label in zip(training_pairs, labels):
        roles = role_transform_tokens(
            tokens[pair[0]], tokens[pair[1]]
        )
        for role in roles:
            evidence[role][
                "positive" if int(label) == 1 else "negative"
            ] += 1
    safe = {
        role
...
def train_model(
    ...
    def select_threshold(
        ...
        if selected is None:
            ...
            selected = {
                ...
                "falsePositive": false_positive,
                "precision": (
                    true_positive / accepted if accepted else None
                ),
                "recall": (
                    true_positive / int((labels == 1).sum())
                    if int((labels == 1).sum())
                    else None
                ),
            }
    ...
    lanes = {
        "general": select_threshold(
            "general", lane_masks["general"], 0.90
        ),
    ...
    selected["folds"] = 5
    selected["lanes"] = {
        name: dict(row) for name, row in lanes.items()
    }
...
class UnionFind:
    ...
    def union(self, left: int, right: int) -> tuple[bool, str | None]:
        ...
        if (
            left_seed is not None
            and right_seed is not None
            and left_seed != right_seed
        ):
...
def write_markdown_inventory(
    ...
    selected = [
        ...
        if include_singletons or int(cluster["uniqueTags"]) > 1
    ]
    selected.sort(
        key=lambda row: (
    ...
    lines = [
        ...
        f"- Listed tags: **"
        f"{sum(int(row['uniqueTags']) for row in selected):,}**",
        "",
    ]
    for cluster in selected:
        ...
        lines.extend(
            [
                ...
                f"- Manual seed: "
                f"{markdown_text(cluster.get('manualSeedId') or 'none')}",
                "",
...
def write_assignment_csv(
    path: Path, assignments: list[dict[str, Any]]
) -> None:
    fields = [
        "index",
        "tag",
...
def main() -> None:
    ...
    if MANUAL_WORKBOOK is not None:
        ...
        manual_source = list(
            jsonl(MANUAL_PASS / "node_assignments.jsonl")
        )
        if len(manual_source) != len(records):
            raise RuntimeError(
                "Manual assignment source does not align with records"
            )
        seed_index = json.loads(
            (MANUAL_PASS / "node_seed_index.json").read_text(
                encoding="utf-8"
            )
        )
        seed_meta = {
            str(row["seedClusterId"]): row for row in seed_index
        }
        ...
        if sum(seed is not None for seed in manual_seed) != 1000:
            raise RuntimeError(
                "Expected exactly 1,000 reviewed manual node tags"
            )
    ...
    token_document_frequency = Counter(
        token for row_tokens in tokens for token in set(row_tokens)
    )
    token_idf = {
    ...
    candidate_pairs, candidate_report = build_candidate_pairs(
        records, tokens, grams
    )
    word_matrix, char_matrix, text_report = build_text_features(
        records, normalized_text
    )
    base_vectors = np.load(
        FIRST_PASS / "node_base_vectors.npy", mmap_mode="r"
    )
    context_vectors = np.load(
        FIRST_PASS / "node_context_vectors.npy", mmap_mode="r"
    )
    fused_vectors = np.load(
        FIRST_PASS / "node_fused_vectors.npy", mmap_mode="r"
    )
    ...
    safe_role_tokens, role_evidence = learn_safe_role_tokens(
        training_pairs, labels, tokens
    )
    model, calibration = train_model(training_features, labels)
    threshold = float(calibration["threshold"])
    lane_thresholds = {
        name: float(row["threshold"])
        for name, row in calibration["lanes"].items()
    }

    all_candidate_features = np.empty(
        (len(candidate_pairs), len(FEATURE_NAMES)), dtype=np.float32
    )
    all_candidate_probabilities = np.empty(
        len(candidate_pairs), dtype=np.float32
    )
    batch_size = 10000
    for start in range(0, len(candidate_pairs), batch_size):
        ...
        all_candidate_probabilities[start:stop] = model.predict_proba(
            features
        )[:, 1]
    ...
    for pair_index, (left, right) in enumerate(candidate_pairs):
        ...
        rare_shared_anchor = any(
            token_document_frequency[token] <= 50
    ...
    bootstrapped_safe_roles = {
        ...
        if count >= 2
        or (
            bootstrap_role_with_of_evidence[role] >= 1
            and token_document_frequency[role] >= 50
        )
    }
    ...
    for pair_index, pair in enumerate(candidate_pairs):
        ...
        if not numeric_equal and not (
            manual_seed[left] is not None
            and manual_seed[left] == manual_seed[right]
        ):
            rejected_numeric += 1
            continue
        if normalized_exact:
            ...
            safe_role_reordering = bool(
                set(roles) & safe_role_tokens
            )
            conjunction_reordering = (
                pure_permutation
                and strict_conjunction_reordering(
                    left_tokens, right_tokens
                )
            )
            direct_manual_attachment = (
                (manual_seed[left] is None)
                != (manual_seed[right] is None)
            )
            token_reordering = (
            ...
            orthographic_variant = (
                orthographic_variant_compatible(
                    left_tokens, right_tokens
                )
            ...
            accepted_lane = (
                ...
                if (
                    token_reordering
                    and probability
                    >= lane_thresholds["token_reordering"]
                )
                else "orthographic_variant"
                if (
                    orthographic_variant
                    and probability
                    >= lane_thresholds["orthographic_variant"]
                )
                else "general"
                if (
                    (
            ...
            accepted = (
                ...
                and (
                    not pure_permutation
                    or safe_role_reordering
                    or conjunction_reordering
                )
                and float(features[0]) >= 0.82
                and float(features[2]) >= 0.82
                and (
                    float(features[4]) >= 0.55
                    or float(features[3]) >= 0.70
                )
            )
            method = (
                (
    ...
    for seed_id, members in manual_members.items():
        ...
        for member in members[1:]:
            ...
            if not merged:
                raise RuntimeError(
                    f"Could not initialize manual seed {seed_id}: {reason}"
                )
    ...
    for edge in accepted_edges:
        ...
        right = int(edge["right"])
        if (
            left in ambiguous_direct
            and manual_seed[right] is not None
        ) or (
            right in ambiguous_direct
            and manual_seed[left] is not None
        ):
    ...
    for ordinal, members in enumerate(new_multi, start=1):
        cluster_specs.append(
            (f"NODE_ALIAS_NEW_{ordinal:05d}", None, members)
        )
    for ordinal, members in enumerate(singletons, start=1):
        cluster_specs.append(
            (f"NODE_SINGLETON_{ordinal:05d}", None, members)
        )
        ...
        clusters.append(
            {
                ...
                "mentionCount": sum(
                    int(records[index]["frequency"]) for index in members
                ),
                "reviewedMemberCount": sum(
                    manual_seed[index] is not None for index in members
                ),
                "automaticMemberCount": sum(
                    manual_seed[index] is None for index in members
                ),
    ...
    assignments = [
        assignment_by_index[index] for index in range(len(records))
    ]
    ...
    write_assignment_csv(
        OUTPUT / "node_identity_assignments.csv", assignments
    )
    ...
    report = {
        ...
        "manualReviewedTags": sum(
            seed is not None for seed in manual_seed
        ),
        "manualSeedClusters": len(manual_members),
        ...
        "bootstrappedRoleTokenEvidence": dict(
            sorted(bootstrap_role_evidence.items())
        ),
        "bootstrappedRoleWithOfEvidence": dict(
            sorted(bootstrap_role_with_of_evidence.items())
        ),
