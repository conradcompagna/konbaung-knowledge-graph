...
def write(name,txt):(O/name).write_text(txt,encoding='utf-8')
summary=json.loads((RESULTS/'10_audit_summary.json').read_text());families=pd.read_csv(RESULTS/'10_family_audit.csv').fillna('');methods=pd.read_csv(RESULTS/'10_requested_method_checklist.csv').fillna('');validation=json.loads((RESULTS/'11_validation.json').read_text())
audit=f'''# Audit of existing Konbaung analyses
...
for r in families.itertuples():
 audit+=f'### {r.family} {r.family_name}\n\n**Before:** {r.prior_status}. {r.prior_coverage}\n\n'
 if r.prior_evidence:audit+='**Located evidence:**\n\n'+''.join('- `'+p+'`\n' for p in r.prior_evidence.split(' | '))+'\n'
 audit+='**New work:** '+r.new_work+'\n\n'
 if r.new_outputs:audit+='**Outputs:** '+', '.join(f'[{Path(p).name}]({p})' for p in r.new_outputs.split(';'))+'.\n\n'
 if r.remaining_limits:audit+='**Boundary:** '+r.remaining_limits+'\n\n'
...
ll=pd.read_csv(RESULTS/'01_loglinear_models.csv');cells=pd.read_csv(RESULTS/'01_threeway_cells.csv');supported=cells[(cells.observed>=20)&(cells.expected_all_pairwise>=5)].head(12)
q=pd.read_csv(RESULTS/'02_layer_qap.csv');q=q[q.null_model=='ontology_stratified'];sens=pd.read_csv(RESULTS/'06_layer_qap_sensitivity.csv');noking=sens[sens.specification=='without_sovereign'];q=q.merge(noking[['r1','r2','observed','q_two_sided']],on=['r1','r2'],suffixes=('_all','_no_sovereign'))
perm=json.loads((RESULTS/'01_conditional_permutation.json').read_text());cp=json.loads((RESULTS/'03_static_cp_fit.json').read_text());stability=pd.read_csv(RESULTS/'06_tensor_component_stability.csv');mot=pd.read_csv(RESULTS/'09_category_conditioned_chain_null.csv');focal=['allegiance -> authorization','information -> authorization','authorization -> execution','resistance -> coercion'];rules=pd.read_csv(RESULTS/'04_length2_horn_rules.csv');sem=pd.read_csv(RESULTS/'03_semantic_structure_qap.csv');pred=pd.read_csv(RESULTS/'08_link_prediction.csv');p1=pd.read_csv(RESULTS/'05_p1_models.csv');reigns=pd.read_csv(RESULTS/'06_adjacent_reign_tensor_comparison.csv')
results=f'''# New analyses of power relations in the Konbaung triples
...
'''+table(ll[['model','G2','iterations','max_margin_error','converged']])+f'''
...
'''+table(supported[['S','P','O','observed','expected_all_pairwise','observed_expected']])+f'''
...
'''+table(q[['r1','r2','observed_all','q_two_sided_all','observed_no_sovereign','q_two_sided_no_sovereign']])+'''
...
'''+table(mot[(mot.pattern.isin(focal))&(mot.unit=='path_count')][['pattern','observed','null_mean','p_greater','q_greater']])+'''
...
'''+table(rules[['body1','body2','head','train_support','train_pages','train_confidence','test_body_pairs','test_support','test_confidence']])+'''
...
The script order is recorded in [RUN_ANALYSES.ps1](RUN_ANALYSES.ps1). The exact source archive and input hashes are in [original_source_verification.json](original_source_verification.json), [input_provenance.json](input_provenance.json), and the original source manifest. The environment, seeds and **'''+str(validation['checks_passed'])+''' passing invariants** are recorded in [11_validation.json](results/11_validation.json) and [11_environment.json](results/11_environment.json). Four figures are supplied as PNG and SVG. The [selected source passages](results/09_selected_source_passages.csv) retain Burmese and English; the HTML source reader links them to sentence IDs.
...
def inline(s):
 ...
 s=re.sub(r'<code>(vol[123]_s\d+)</code>',r'<a href="source_passages.html#\1"><code>\1</code></a>',s)
...
def mdhtml(text):
 ...
 while i<len(lines):
  ...
  if line.startswith('| '):
   ...
   out.append('<div class="tablewrap"><table><thead><tr>'+''.join('<th>'+inline(c.strip())+'</th>' for c in chunk[0].strip('|').split('|'))+'</tr></thead><tbody>')
   for row in chunk[2:]:out.append('<tr>'+''.join('<td>'+inline(c.strip())+'</td>' for c in row.strip('|').split('|'))+'</tr>')
   out.append('</tbody></table></div>');continue
  m=re.match(r'(#+) (.*)',line)
  if m:level=min(4,len(m[1]));out.append(f'<h{level}>'+inline(m[2])+f'</h{level}>');i+=1;continue
  m=re.match(r'!\[([^\]]*)\]\(([^)]+)\)',line)
  if m:out.append(f'<figure><img alt="{html.escape(m[1])}" src="{html.escape(m[2])}"></figure>');i+=1;continue
  if line.startswith('- '):
   out.append('<ul>')
...
methodtable=methods[['family','family_name','requested_method','status','detail']].to_html(index=False,escape=True,table_id='methodtable')
sections='<section id="interpretation" class="tab">'+mdhtml(interpretation)+'</section><section id="results" class="tab" hidden>'+mdhtml(results)+'</section><section id="audit" class="tab" hidden>'+mdhtml(audit)+'</section>'
options=''.join('<option>'+html.escape(x)+'</option>' for x in sorted(methods.status.unique()))
sections+='<section id="methods" class="tab" hidden><h1>Method enumeration</h1><p>All 380 bullet items from the supplied catalogue, including overlapping metrics and unrun alternatives.</p><div class="tools"><input id="search" aria-label="Search methods" placeholder="Search methods or families"><select id="status" aria-label="Filter status"><option value="">All statuses</option>'+options+'</select><span id="count" class="muted"></span></div><div class="tablewrap">'+methodtable+'</div></section>'
...
write('index.html','<!doctype html><html lang="en"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Konbaung power relations audit and results</title><style>'+css+'</style><header><p class="muted">11 September 2026 · Verified canonical corpus · Reproducible analysis</p><nav><button class="active" data-tab="interpretation">Historical interpretation</button><button data-tab="results">New results</button><button data-tab="audit">Existing work audit</button><button data-tab="methods">380-item checklist</button><a href="source_passages.html">Source passages</a></nav></header><main>'+sections+'</main><script>'+js+'</script></html>')
passages=pd.read_csv(RESULTS/'09_selected_source_passages.csv').fillna('');content='<h1>Selected source passages</h1><p>Canonical Burmese and supplied English translations. These are model-assisted translations; the source is retained for historical reading. Sentence IDs connect to the result ledgers. <a href="index.html">Return to the report</a>.</p>'
for r in passages.itertuples():content+='<details id="'+html.escape(r.sentence_id)+'"><summary>'+html.escape(f'{r.sentence_id} · {r.volume} page {r.page}')+'</summary><p lang="my">'+html.escape(r.burmese)+'</p><p>'+html.escape(r.english_translation)+'</p></details>'
write('source_passages.html','<!doctype html><html lang="en"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Konbaung source passages</title><style>'+css+'</style><main>'+content+'</main></html>')
write('RUN_ANALYSES.ps1', '''$ErrorActionPreference = 'Stop'
...
recovery=json.loads((O/'source_recovery.json').read_text());recovery['nested_member']='Konbaung_Status_Power_FINAL_Statistical_Reproducibility_UPDATED/02_CANONICAL_SOURCE/DIGHUM_WEBGPT_ANALYSIS_PACKAGE_20260831.zip';recovery['resolution']='Intact original archive located and verified; used for new analyses.';(O/'source_recovery.json').write_text(json.dumps(recovery,indent=2))
