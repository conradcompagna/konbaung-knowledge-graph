        ...
        if "xl/sharedStrings.xml" in archive.namelist():
            ...
            for item in root.findall("m:si", namespaces):
                shared.append(
                    "".join(
                        node.text or ""
                        for node in item.findall(".//m:t", namespaces)
                    )
                )
            ...
            for row in root.findall("m:sheetData/m:row", namespaces):
                ...
                for cell in row.findall("m:c", namespaces):
                    ...
                    if cell_type == "s" and value_node is not None:
                        ...
                        value = "".join(
                            node.text or ""
                            for node in inline_node.findall(".//m:t", namespaces)
                        )
...
def effective_seeds(
    ...
    if len(assignments) != 1000:
        raise RuntimeError(
            f"Expected 1,000 manual {kind} assignments, found {len(assignments)}"
        )
    ...
    cluster_member_counts = Counter(
        str(row["cluster_id"]) for row in assignments
    )
    ...
    if workbook_cluster_mismatches:
        raise RuntimeError(
            f"Workbook {kind} cluster/member mismatch: "
            f"{workbook_cluster_mismatches[:10]}"
        )
...
def build_head_matrix(
    ...
    heads = [
        (
            str(row["normalizedTag"]).split()[0]
            if str(row["normalizedTag"]).split()
            else ""
        )
        for row in records
    ]
...
def head_match(
    ...
    candidates = [
        ...
        and (
            ...
            or direction_signatures[index]
            == direction_signatures[record_index]
        )
    ]
    if not candidates:
        return 0.0, None
    scores = np.asarray(
        (head_matrix[candidates] @ head_matrix[record_index].T).toarray()
    ).reshape(-1)
...
def calibrate_thresholds(
    ...
    eligible_seed_indices = [
        int(seed["seedIndex"])
        for seed in seeds
        if seed["action"] in {"MERGE", "REVIEW_MERGE"}
    ]
    eligible_position = {
        seed_index: position
        for position, seed_index in enumerate(eligible_seed_indices)
    }
    eligible_centroids = np.asarray(
        centroids[eligible_seed_indices], dtype=np.float32
    )
    ...
    for seed in seeds:
        ...
        for local, member_index in enumerate(members):
            ...
            similarities = np.asarray(
                vectors[member_index], dtype=np.float32
            ) @ eligible_centroids.T
            ...
            similarities[true_position] = float(
                np.asarray(vectors[member_index], dtype=np.float32)
                @ leave_one_out
            )
            ...
            evaluations.append(
                {
                    ...
                    "predictedSeedManualNeighborCount": len(
                        supporting_neighbors
                    ),
    ...
    for score_threshold in np.arange(minimum, 0.991, 0.005):
        for margin_threshold in np.arange(0.0, 0.151, 0.005):
            for lexical_threshold in np.arange(
                minimum_lexical, 0.991, 0.01
            ):
                accepted = [
                    row
                ...
                if best is None or key > best:
                    ...
                    selected = {
                        ...
                        "lexicalThreshold": round(
                            float(lexical_threshold), 6
                        ),
...
def automatic_seed_assignments(
    ...
    centroid_transpose = np.asarray(
        centroids[eligible_seed_indices].T, dtype=np.float32
    )
    ...
    neighbor_similarities = knn["similarities"]
    unseeded = [
        index for index in range(len(records)) if index not in manual_assignment
    ]
    for start in range(0, len(unseeded), 512):
        ...
        partition_scores = np.take_along_axis(
            similarities, partitioned, axis=1
        )
        ...
        for local, record_index in enumerate(indices):
            best_seed = eligible_seed_indices[
                int(ordered_indices[local, 0])
            ]
            ...
            row = {
                ...
                "lexicalWordSimilarity": round(
                    lexical_word_similarity, 6
                ),
                "lexicalCharSimilarity": round(
                    lexical_char_similarity, 6
                ),
                ...
                "headBestManualMemberTag": (
                    records[head_member_index]["tag"]
                    if head_member_index is not None
                    else None
                ),
                ...
                "bestManualNeighborSimilarity": (
                    round(
                        max(value for _, value in supporting_neighbors), 6
                    )
                    if supporting_neighbors
                    else None
                ),
    ...
    summary = {
        ...
        "passiveByDirectionCompatibilityRequired": (
            direction_signatures is not None
        ),
        "scoreQuantiles": {
            str(quantile): float(
                np.quantile([row["score"] for row in score_rows], quantile)
            )
            for quantile in (0.1, 0.25, 0.5, 0.75, 0.9, 0.95, 0.99)
        },
        "marginQuantiles": {
            str(quantile): float(
                np.quantile([row["margin"] for row in score_rows], quantile)
            )
            for quantile in (0.1, 0.25, 0.5, 0.75, 0.9, 0.95, 0.99)
        },
...
def residual_communities(
    ...
    for source in residual:
        for rank in range(min(int(config["neighbor_count"]), indices.shape[1])):
            ...
            if similarity < minimum:
                continue
            weight = max(
                1e-6, (similarity - minimum) / max(1e-6, 1.0 - minimum)
            )
            if (
                not graph.has_edge(source, target)
                or weight > graph[source][target]["weight"]
            ):
                graph.add_edge(
                    source,
                    target,
    ...
    report = {
        ...
        "recordsInCommunitiesUnder5": sum(
            len(group) for group in communities if len(group) < 5
        ),
    }
...
def materialize_outputs(
    ...
    for seed in seeds:
        ...
        similarities = {
            index: float(
                np.asarray(vectors[index], dtype=np.float32) @ centroid
            )
        ...
        for index in automatic_members:
            ...
            assignment_by_index[index] = {
                ...
                "lexicalBestManualMemberTag": proposal[
                    "lexicalBestManualMemberTag"
                ],
                "headSimilarity": proposal["headSimilarity"],
                "headBestManualMemberTag": proposal[
                    "headBestManualMemberTag"
                ],
            }
...
def run_kind(
    ...
    vectors = np.load(
        FIRST_PASS_ROOT / f"{kind}_fused_vectors.npy", mmap_mode="r"
    )
    seeds, manual_assignment, manual_validation = effective_seeds(
        kind, sheets, records
    )
    centroids = build_centroids(vectors, seeds)
    word_matrix, char_matrix, lexical_config = build_lexical_matrices(
        records
    )
    direction_signatures = (
        [
            "by" in str(row["normalizedTag"]).split()
            for row in records
        ]
        if kind == "edge"
        else None
    ...
    communities, residual_report = residual_communities(
    ...
    write_assignment_csv(
        OUTPUT_ROOT / f"{kind}_assignments.csv", assignments
    )

    manual_preserved = all(
        assignments[index]["clusterId"]
        == seeds[seed_index]["seedClusterId"]
    ...
    return {
        ...
        "outputFiles": {
            ...
            "assignmentsJsonl": str(
                OUTPUT_ROOT / f"{kind}_assignments.jsonl"
            ),
            "assignmentsCsv": str(
                OUTPUT_ROOT / f"{kind}_assignments.csv"
            ),
            "centroids": str(
                OUTPUT_ROOT / f"{kind}_seed_centroids.npy"
            ),
        },
    }
...
def report_markdown(result: dict[str, Any]) -> str:
    lines = [
        ...
        "## Method",
        "",
        "1. The 1,000 reviewed tags in each domain are fixed to their workbook "
        "clusters.",
    ...
    for kind in ("node", "edge"):
        ...
        lines.extend(
            [
                ...
                f"- Workbook assignments fixed: **"
                f"{validation['manualAssignments']:,}**",
                f"- Workbook clusters: **{validation['manualClusters']:,}**",
                f"- Effective seed centroids: **"
                f"{validation['effectiveSeedCentroids']:,}**",
                f"- Expandable `MERGE`/`REVIEW_MERGE` centroids: **"
                f"{validation['expandableSeedCentroids']:,}**",
                f"- Fixed singleton centroids: **"
                f"{validation['fixedSingletonSeedCentroids']:,}**",
                ...
                f"- Selected centroid score threshold: **"
                f"{calibration['scoreThreshold']:.3f}**",
                f"- Selected margin threshold: **"
                f"{calibration['marginThreshold']:.3f}**",
                f"- Selected lexical threshold: **"
                f"{calibration['lexicalThreshold']:.3f}**",
                f"- Predicate-head threshold: **"
                f"{calibration['domainMinimumHeadSimilarity']:.3f}**",
                ...
                f"- Tags left for residual Louvain: **"
                f"{residual['residualRecords']:,}**",
...
def main() -> None:
    ...
    parser.add_argument(
        "--kind", choices=("node", "edge", "all"), default="all"
    )
