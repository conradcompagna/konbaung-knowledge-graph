...
class RelationAnnotation(BaseModel):
    relations: list[RelationOut] = Field(description="Positive relation labels between Call 1 span IDs.")
...
def build_page_index_with_neighbors(source_root: Path, jobs: list[PageJob]) -> dict[tuple[str, int], PageJob]:
...
def context_blocks(job: PageJob, page_index: dict[tuple[str, int], PageJob], context_ratio: float) -> dict[str, str]:
    ...
    return {
        "previous_context": context_excerpt(previous_job.raw_text, context_chars, "previous") if previous_job else "",
...
def build_entity_prompt(job: PageJob, page_index: dict[tuple[str, int], PageJob], context_ratio: float, schema_detail: str) -> str:
...
def build_relation_prompt(
    ...
    return RELATION_PROMPT_TEMPLATE.format(
        ...
        volume_id=job.volume_id,
        page_num=f"{job.page_num:04d}",
        entity_spans_json=json.dumps(relation_prompt_spans(entity_record), ensure_ascii=False, indent=2),
        **blocks,
    )
...
def without_all_spaces(text: str, positions: list[Optional[tuple[int, int]]]) -> tuple[str, list[Optional[tuple[int, int]]]]:
    chars: list[str] = []
    kept_positions: list[Optional[tuple[int, int]]] = []
    for char, position in zip(text, positions):
        if char.isspace():
    ...
    *,
    ...
    if start == -1:
        ...
        for _ in range(i):
            start = match_text.find(match_needle, search_from)
            if start == -1:
                return None, f"text span not found in target page after whitespace normalization: {tx!r}"
            search_from = start + len(match_needle)
...
def validate_span_annotation(job: PageJob, annotation: SpanAnnotation) -> tuple[dict[str, Any], list[str], list[str]]:
    errors: list[str] = []
    warnings: list[str] = []
    ...
    for idx, span in enumerate(annotation.spans, start=1):
        ...
        resolved_ref, error = resolve_text_ref(job.raw_text, lines, ln=span.ln, tx=span.tx, i=span.i)
        if error:
            errors.append(f"{span_ref} {span.id}: {error}")
            resolved_ref = span.model_dump()
        else:
            key = (str(span.label), int(resolved_ref.get("abs_s", -1)), int(resolved_ref.get("abs_e", -1)))
            if key in seen_locations:
                warnings.append(f"{span_ref} {span.id}: duplicate label/location dropped")
                continue
            seen_locations.add(key)
            if resolved_ref.get("line_hint_corrected"):
                warnings.append(f"{span_ref} {span.id}: line hint corrected from {span.ln} to {resolved_ref.get('ln')}")
...
def validate_relation_annotation(
    ...
    for idx, relation in enumerate(annotation.relations, start=1):
        ...
        resolved_ref, error = resolve_text_ref(job.raw_text, lines, ln=relation.ln, tx=relation.tx, i=relation.i)
        if error:
            ...
            if resolved_ref.get("line_hint_corrected"):
                warnings.append(f"{rel_ref} {relation.id}: line hint corrected from {relation.ln} to {resolved_ref.get('ln')}")
...
def write_entity_records(
    ...
    write_json(stage_output_path(out_dir, "entity_spans", "postprocessed", bucket, job), post_record)
...
def write_relation_records(
    ...
    base = {
        **page_metadata(job),
        ...
        "usage_metadata": usage,
        "prompt_path": str(prompt_path),
        "entity_spans_path": str(stage_output_path(out_dir, "entity_spans", "postprocessed", "valid", job)),
    }
    raw_record = {
        **base,
        "annotation": annotation.model_dump(),
...
def build_review_text(job: PageJob, joined_record: dict[str, Any]) -> str:
    ...
    for relation in joined_record.get("relations", []):
        ...
        if evidence_spans:
            ln = max(part.get("ln", 0) for part in evidence_spans if isinstance(part.get("ln"), int))
        else:
            ln = evidence.get("ln")
...
def write_joined_record(
    ...
    record = {
        **page_metadata(job),
        ...
        "entity_spans": entity_record.get("annotation", {}).get("spans", []) if entity_record else [],
        "relations": relation_record.get("annotation", {}).get("relations", []) if relation_record else [],
...
def prepare_outputs(args: argparse.Namespace, jobs: list[PageJob], page_index: dict[tuple[str, int], PageJob]) -> None:
        ...
        if args.stage in ("relations", "both"):
            for job in prompt_jobs:
                ...
                write_text(
                    ...
                    build_relation_prompt(job, page_index, args.context_ratio, args.schema_detail, entity_record),
                )
    ...
    summary = {
        ...
        "out_dir": str(out_dir),
        "script": "konbaung_gemini_xlmr_seed_annotator.py",
        "schema": {
...
def run_entity_stage(
    ...
    return write_entity_records(out_dir, job, annotation, resolved, errors, warnings, usage, prompt_path)
...
def run_relation_stage(
    ...
    out_dir = Path(args.out_dir)
    prompt = build_relation_prompt(job, page_index, args.context_ratio, args.schema_detail, entity_record)
    ...
    return write_relation_records(out_dir, job, annotation, resolved, errors, warnings, usage, prompt_path, entity_record)
...
def run_annotation(args: argparse.Namespace, jobs: list[PageJob], page_index: dict[tuple[str, int], PageJob]) -> None:
    ...
    for job in tqdm(effective_jobs, desc=f"Running {args.stage}"):
        ...
        try:
            ...
            if args.stage in ("relations", "both"):
                relation_bucket, relation_record = run_relation_stage(args, client, job, page_index, entity_record)
            ...
            stage_dir = "entity_spans" if args.stage in ("entities", "both") and entity_record is None else "relations"
            write_error_record(out_dir, job, stage_dir, exc)
            if args.stage == "both":
                write_joined_record(out_dir, job, entity_record, relation_record, [f"{type(exc).__name__}: {exc}"])
...
def parse_args(argv: Optional[list[str]] = None) -> argparse.Namespace:
    ...
    parser.add_argument("--out-dir", default=str(DEFAULT_OUT_DIR), help="Output directory for prompts and annotations.")
    ...
    parser.add_argument("--volume-id", default=None, help="Optional single volume selector, e.g. konbaung_vol1.")
    parser.add_argument("--page-num", type=int, default=None, help="Optional single OCR page selector, e.g. 177.")
    parser.add_argument("--limit", type=int, default=None, help="Limit jobs for prepare/run.")
    parser.add_argument("--save-prompts", action="store_true", help="Write prompt files without calling Gemini.")
    parser.add_argument("--prompt-limit", type=int, default=25, help="Max prompts to save when --save-prompts is used. Use 0 for all.")
    parser.add_argument("--context-ratio", type=float, default=DEFAULT_CONTEXT_RATIO, help="Previous/next context as a ratio of target page chars.")
    parser.add_argument(
        "--schema-detail",
