...
from pair_classifier_features import (  # noqa: E402
    STOPWORDS, GENERIC_TITLES, GROUP_MARKERS, PERSON_MARKERS, PLACE_MARKERS, EVENT_MARKERS,
    build_corpus_stats,
    feature_vector,
...
OUTPUT_COLUMNS = [
    "raw_rank", "raw_score_percent", "score_band", "left_tag", "right_tag",
    "rank_from_left", "rank_from_right", "raw_model_probability",
    "model_probability", "decision",
...
def deterministic_keys(meta):
    ...
    return {
        ...
        "typed_non_person": bool(meta.objects or meta.events or meta.places or meta.institutions or (meta.groups and not meta.persons)),
...
def deterministic_reason(left, right) -> Optional[str]:
    ...
    safe_reorder = (
        (left["collective"] and right["collective"])
        or (left["typed_non_person"] and right["typed_non_person"] and not left["personal_prefix"] and not right["personal_prefix"])
    )
    ...
    if (left["has_ofthe"] or right["has_ofthe"]) and left["ofthe"] == right["ofthe"] and left["ofthe"]:
        return "same_tokens_after_removing_of_the"
    if (
        left["city_anchor"] and right["city_anchor"]
        and left["has_citytown"] and right["has_citytown"]
        and left["city_base"] == right["city_base"]
    ):
        return "named_city_town_wording_variant"
    if (
        left["event_anchor"] and right["event_anchor"]
        and left["has_eventword"] and right["has_eventword"]
...
def main() -> None:
    ...
    parser.add_argument("--model", type=Path, default=HERE / "pair_classifier_output" / "raw_coreference_pair_classifier.joblib")
    ...
    def flush_batch(writer: csv.DictWriter) -> None:
        if batch_vectors:
            ...
            for record_index, vector, raw_value, value in zip(
                ...
                personal_prefixes = {"nga", "maung", "ma", "u"}
                short_personal_reordering_risk = (
                    bool(left_meta.tokens and left_meta.tokens[0] in personal_prefixes)
                    or bool(right_meta.tokens and right_meta.tokens[0] in personal_prefixes)
                )
                if value >= auto_threshold and identity_evidence_gate(vector) and not short_personal_reordering_risk:
                    record["decision"] = "auto_merge"
                    record["decision_source"] = "ml_high_precision_gate"
                    record["decision_reason"] = "probability_above_auto_threshold_with_lexical_identity_evidence"
        ...
        for raw_rank, candidate in enumerate(iter_candidates(args.candidates), start=1):
            ...
            if candidate.score < model_min_score:
                ...
                if det_reason in exact_reasons:
                    ...
                    record["decision_reason"] = "raw_similarity_below_94_percent_without_deterministic_identity_match"
                ...
                if hard_reason:
                    ...
                    vector = feature_vector(candidate.left, candidate.right, candidate.score, rank_left, rank_right, stats)
    ...
    (args.output_dir / "scoring_summary.json").write_text(json.dumps(summary, indent=2), encoding="utf-8")
    print(json.dumps(summary["decision_counts"], indent=2), flush=True)
    print(f"Wrote {len(auto_rows):,} automatic merges and {len(review_rows):,} manual-review candidates.", flush=True)
