...
OLD_BUNDLE = ROOT / "raw_coreference_pair_classifier_bundle_20260725"
...
REGEX_REVIEW = (
    ROOT
    / "konbaung_node_identity_regex_refinement_20260725"
    / "regex_candidate_adjudication.jsonl"
)
...
def load_sources() -> dict:
    ...
    ledger = read_csv(
        MANUAL / "konbaung_node_identity_merge_ledger_complete.tsv", delimiter="\t"
    )
    node_records = list(read_jsonl(RAW_VECTORS / "node_records.jsonl"))
    corrected_clusters = list(
        read_jsonl(CLUSTER_VECTORS / "corrected_initial_clusters.jsonl")
    )
    ...
    if len(manual_clusters) != EXPECTED_MANUAL_CLUSTERS:
        raise RuntimeError(
            f"Expected {EXPECTED_MANUAL_CLUSTERS:,} manual clusters; "
            f"found {len(manual_clusters):,}"
        )
...
def build_full_gold() -> tuple[pd.DataFrame, CorpusStats, dict, dict]:
    ...
    raw_base = normalize(
        np.load(RAW_VECTORS / "node_base_vectors.npy")[vector_order]
    )
    raw_context = normalize(
        np.load(RAW_VECTORS / "node_context_vectors.npy")[vector_order]
    )
    ...
    corrected_index = {
        row["clusterId"]: index for index, row in enumerate(corrected_clusters)
    }
    corrected_label = {
        row["clusterId"]: row["canonicalLabel"] for row in corrected_clusters
    }
    ...
    def add_supplemental(
        ...
        if key in supplemental:
            ...
            row["source"] = ";".join(
                sorted(set(row["source"].split(";")) | {source})
            )
    ...
    component_to_final = {
        row["source_component_id"]: row["final_cluster_id"]
        for row in component_assignments
    }
    ...
    source_counts = (
        frame.groupby(["source", "label"]).size().rename("pairs").reset_index()
    )
    audit = {
...
def oof_predictions(
    ...
    for repeat in range(repeats):
        splitter = StratifiedGroupKFold(
            n_splits=5, shuffle=True, random_state=20260725 + repeat
        )
...
def choose_review_threshold(
    ...
    for threshold in sorted(np.unique(probability), reverse=True):
        ...
        if recall >= target_recall:
            ...
            precision = (
                weighted_true_positive / weighted_selected if weighted_selected else 0.0
            )
            best = (
                float(threshold),
                {
...
def train(repeats: int) -> tuple[dict, pd.DataFrame, dict]:
    ...
    pd.DataFrame(extras["conflicts"]).to_csv(
        OUTPUT / "excluded_label_conflicts.csv", index=False
    )
    ...
    print(
        f"Running {repeats} x 5 identity-family-disjoint CV over "
        f"{len(frame):,} gold pairs...",
        flush=True,
    )
    ...
    comparison = {
        ...
        "logistic_regression": {
            ...
            "identity_weighted": metrics(
                y, logistic_oof, evaluation_weights
            ),
        },
        "hist_gradient_boosting": {
            "unweighted": metrics(y, boosted_oof),
            "identity_weighted": metrics(
                y, boosted_oof, evaluation_weights
            ),
        },
    }
    logistic_ap = comparison["logistic_regression"]["identity_weighted"][
        "average_precision"
    ]
    boosted_ap = comparison["hist_gradient_boosting"]["identity_weighted"][
        "average_precision"
    ]
...
def write_training_report(summary: dict) -> None:
    ...
    cluster_validation = summary["granularity_validation"]["cluster_pair"][
        "selected_model_oof"
    ]
    ...
    lines.extend(
        [
            ...
            "## Operating policy",
            "",
            "ML automatic merging remains disabled. The classifier ranks a manual "
            "review queue.",
            "",
...
def score(bundle: dict | None = None, batch_size: int = 25_000) -> dict:
    ...
    cluster_context = normalize(
        np.load(CLUSTER_VECTORS / "context_cluster_centroids.npy")
    )
    ...
    def flush(writer: csv.DictWriter) -> None:
        ...
        for row, raw_value, value in zip(batch_rows, raw_probability, probability):
            ...
            key = tuple(
                sorted((row["left_cluster_id"], row["right_cluster_id"]))
            )
            known_label = known_pair_labels.get(key)
            row["known_gold_label"] = (
                str(known_label) if known_label is not None else ""
            )
            ...
            if key in conflict_pairs:
                ...
                row["decision"] = (
                    "manual_review" if float(value) >= threshold else "reject"
                )
            writer.writerow(row)
    ...
    with gzip.open(all_path, "wt", encoding="utf-8", newline="", compresslevel=1) as handle:
        ...
        for rank, candidate in enumerate(read_jsonl(RAW_CANDIDATES), start=1):
            ...
            batch_features.append(
                stable_feature_vector(left_tag, right_tag, similarities, stats)
            )
            batch_rows.append(
                {
                    "raw_rank": rank,
    ...
    if rank != EXPECTED_RAW_CANDIDATES:
        raise RuntimeError(
            f"Expected {EXPECTED_RAW_CANDIDATES:,} scored rows; found {rank:,}"
        )
    ...
    eligible_positive = {
        key
        for key in known_positive
        if key[0] in corrected_index and key[1] in corrected_index
    }
    pre_resolved_positive = known_positive - eligible_positive
    eligible_negative = {
        key
        for key in known_negative
        if key[0] in corrected_index and key[1] in corrected_index
    }
    summary = {
        "pairs_scored": rank,
        ...
        "known_cluster_pair_retrieval": {
            ...
            "positive_pairs_already_resolved_before_retrieval": len(
                pre_resolved_positive
            ),
            "eligible_unresolved_positive_pairs": len(eligible_positive),
            "positive_pairs_present_in_top100_pool": len(
                eligible_positive & retrieved_known
            ),
            ...
            "negative_pairs_present_in_top100_pool": len(
                eligible_negative & retrieved_known
            ),
        },
        "outputs": {
