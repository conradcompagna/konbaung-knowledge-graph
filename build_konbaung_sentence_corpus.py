...
GREGORIAN_YEAR_RE = re.compile(r"(?:1[4-9][0-9]{2}|2[0-9]{3}|[\u1041\u1042][\u1044-\u1049\u101d][\u1040-\u1049\u101d]{2})")
...
def is_publisher_line(text: str) -> bool:
    ...
    if len(value) > 40:
        return False
    return value.startswith(("\u101b\u102c\u1015\u103c\u100a\u1037", "\u1005\u102c\u1015\u103c\u100a\u1037")) and (
        "\u1021\u102f\u1015\u103a" in value
        or "\u1010\u102d\u102f\u1000\u103a" in value
        or "\u1000\u102d\u102f\u1000\u103a" in value
        or "\u1010\u102d\u102f\u1037" in value
...
def is_footnote_signal(text: str) -> bool:
    ...
    if EDITOR_MARKER in stripped:
        return True
    return bool(NUMBERED_RE.match(stripped) and (is_date_reference(stripped) or LATIN_RE.search(stripped)))
...
def is_footnote_lead(text: str) -> bool:
    ...
    return bool(
        ...
        or (NUMBERED_RE.match(stripped) and len(stripped) <= 24 and NUMERIC_FRAGMENT_RE.fullmatch(stripped))
...
def classify_removed_lines(text: str) -> tuple[set[int], dict[int, str], list[dict[str, Any]]]:
    ...
    footer_limit = publisher_index if publisher_index is not None else (nonempty[-1] + 1 if nonempty else 0)
...
def build_volume(volume: int, files: list[Path], output: Path) -> dict[str, Any]:
    ...
    tail_range = trimmed_range(last_page["cleaned_text"], tail_start, len(last_page["cleaned_text"]))
    ...
    jsonl_write(output / "cross_page_sentences" / f"vol{volume}.jsonl", (s for s in sentences if s["cross_page"]))
...
def validate(results: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for result in results:
        ...
        for record in records:
            reconstructed = normalize_whitespace(" ".join(piece["text"] for piece in record["source"]))
            ...
            for piece in record["source"]:
                ...
                if exact != piece["source_text"]:
                    errors.append(f"{record['id']}: source substring mismatch on page {piece['page']}")
                if normalize_whitespace(exact) != normalize_whitespace(piece["text"]):
                    errors.append(f"{record['id']}: normalized source mismatch on page {piece['page']}")
        ...
        for page in result["pages"]:
            for index, char in enumerate(page["cleaned_text"]):
                ...
                if count != 1:
                    errors.append(f"vol{result['volume']} page {page['page']} char {index}: assigned {count} times")
                    if len(errors) >= 100:
                        break
            for removal in page["removals"]:
                exact = page["original_text"][removal["start_codepoint"] : removal["end_codepoint"]]
                if exact != removal["text"]:
                    errors.append(f"vol{result['volume']} page {page['page']}: removal source mismatch")
        ...
        for result in results:
            for page in result["pages"]:
                for item in page["removals"]:
                    ...
                    handle.write(f"{result['volume']}\t{page['page']}\t{item['line']}\t{item['reason']}\t{text}\n")
    with (review / "possible_footnotes.tsv").open("w", encoding="utf-8", newline="\n") as handle:
        handle.write("volume\tpage\tline\treason\ttext\n")
        for result in results:
            for item in result["possible_footnotes"]:
                text = item["text"].replace("\t", " ")
                handle.write(f"{item['volume']}\t{item['page']}\t{item['line']}\t{item['reason']}\t{text}\n")
