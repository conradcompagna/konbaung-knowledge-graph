...
import konbaung_v3_eight_view_embeddings as batch_run
...
def contents_for(rows: list[dict[str, Any]]) -> list[types.Content]:
    return [
        types.Content(parts=[types.Part(text=extract_text(row))]) for row in rows
    ]
...
def create_or_open_state(
    ...
    if path.exists():
        ...
        if vectors.shape != (len(rows), DIMENSIONS):
            raise RuntimeError(
                f"Unexpected checkpoint shape for {shard}: {vectors.shape}"
            )
        return state, vectors

    if vec_path.exists() and not manifest_path(shard).exists():
        raise RuntimeError(
            f"Unclaimed vector file already exists for {shard}: {vec_path}"
        )
...
def certify_shard(
    ...
    if float(norms.min()) < 0.99 or float(norms.max()) > 1.01:
        raise RuntimeError(
            f"Unexpected vector norms in {shard}: "
            f"{float(norms.min())}..{float(norms.max())}"
        )
    ...
    standard_cost = (
        tokens / 1_000_000 * STANDARD_PRICE_PER_MILLION_TOKENS_USD
    )
    manifest = {
        "collectedAt": now_iso(),
        "shard": shard,
...
def run_shard(shard: str, token_window: TokenWindow) -> None:
    ...
    while int(state["nextRow"]) < len(rows):
        ...
        pending_group_cost = (
            token_count / 1_000_000 * STANDARD_PRICE_PER_MILLION_TOKENS_USD
        )
        if current_cost + pending_group_cost > TOTAL_RUN_COST_CEILING_USD:
            raise RuntimeError(
                f"Hard cost ceiling would be exceeded before {shard}:{start}-{end}"
            )
        ...
        for offset, embedding in enumerate(embeddings):
            ...
            if len(values) != DIMENSIONS:
                raise RuntimeError(
                    f"{shard}:{start + offset} returned {len(values)} dimensions"
                )
        ...
        state["retryCount"] = (
            int(state["retryCount"]) + count_retries + embed_retries
        )
...
def finalize_mixed_run() -> None:
    ...
    request_keys = {
        row["key"] for row in batch_run.iter_jsonl(OUTPUT_ROOT / "request_index.jsonl")
    }
    collected_keys: set[str] = set()
    for shard in batch_run.SHARD_ORDER:
        for row in batch_run.iter_jsonl(
            OUTPUT_ROOT / "vectors" / f"{shard}.keys.jsonl"
        ):
            key = row["key"]
    ...
    batch_cost = sum(
        float(item.get("actualBatchCostUsd", 0.0)) for item in manifests
    )
    standard_cost = sum(
        float(item.get("actualStandardCostUsd", 0.0)) for item in manifests
    )
    total_cost = batch_cost + standard_cost
    if total_cost > TOTAL_RUN_COST_CEILING_USD:
        raise RuntimeError(
            f"Final cost ${total_cost:.4f} exceeded "
            f"${TOTAL_RUN_COST_CEILING_USD:.2f} ceiling"
        )
