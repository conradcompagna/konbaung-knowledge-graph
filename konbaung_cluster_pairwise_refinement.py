...
import konbaung_v3_node_disambiguation as identity_model


ROOT = Path(__file__).resolve().parent
BUNDLE = (
    ROOT / "konbaung_node_identity_disambiguation_complete_bundle_20260725"
)
...
def exact_centroid_knn(
    ...
    corpus = torch.from_numpy(
        np.ascontiguousarray(normalize_rows(matrix))
    ).to(device)
    result = np.empty(
        (matrix.shape[0], neighbors), dtype=np.int32
    )
    with torch.inference_mode():
        for start in range(0, matrix.shape[0], batch_size):
            ...
            similarities[local_rows, torch.arange(start, stop, device=device)] = (
                -2.0
            )
            indices = torch.topk(
                similarities, k=neighbors, dim=1, largest=True
            ).indices
...
def build_cluster_state(
    ...
    for index, row in enumerate(assignments):
        ...
        if str(records[index]["tag"]) != row["display_tag"]:
            raise RuntimeError(
                f"Tag/vector alignment mismatch at rank {row['rank']}"
            )
        ...
        canonical_by_cluster.setdefault(
            cluster_id, row["canonical_label"]
        )
    ...
    base_centroids = np.empty(
        (len(cluster_ids), base_vectors.shape[1]), dtype=np.float32
    )
    context_centroids = np.empty(
        (len(cluster_ids), context_vectors.shape[1]), dtype=np.float32
    )
    fused_centroids = np.empty(
        (len(cluster_ids), fused_vectors.shape[1]), dtype=np.float32
    )
    for cluster_index, cluster_id in enumerate(cluster_ids):
        ...
        base_centroids[cluster_index] = np.asarray(
            base_vectors[members], dtype=np.float32
        ).mean(axis=0)
        context_centroids[cluster_index] = np.asarray(
            context_vectors[members], dtype=np.float32
        ).mean(axis=0)
        fused_centroids[cluster_index] = np.asarray(
            fused_vectors[members], dtype=np.float32
        ).mean(axis=0)
        display_tags = [
            assignments[index]["display_tag"] for index in members
        ]
        clusters.append(
            {
                ...
                "totalFrequency": sum(
                    int(assignments[index]["frequency"])
...
def build_cluster_candidates(
    ...
    centroid_knn = exact_centroid_knn(
        fused_centroids, CENTROID_NEIGHBORS
    )
    for left in range(centroid_knn.shape[0]):
        for right in centroid_knn[left]:
            add_candidate(
                sources, left, int(right), "fused_centroid_knn"
            )
    ...
    for index, cluster in enumerate(clusters):
        tokens = identity_model.normalized_tokens(
            str(cluster["canonicalLabel"])
        )
...
def train_tag_pair_model(
    ...
    manual_seed = [
        redirected(row["final_cluster_id"]) for row in assignments
    ]
    ...
    tokens = [
        identity_model.normalized_tokens(str(row["tag"]))
        for row in records
    ]
    normalized_text = [" ".join(row) for row in tokens]
    numeric = [
        identity_model.numeric_signature(row) for row in tokens
    ]
    grams = [
        identity_model.character_ngrams(row) for row in tokens
    ]
    token_document_frequency = Counter(
        token for row in tokens for token in set(row)
    )
    token_idf = {
        token: np.log(
            (len(records) + 1)
    ...
    tag_candidates, candidate_report = (
        identity_model.build_candidate_pairs(records, tokens, grams)
    )
    word_matrix, char_matrix, text_report = (
        identity_model.build_text_features(records, normalized_text)
    )
    ...
    training_pairs, labels, training_report = (
        identity_model.manual_training_pairs(
...
def cluster_pair_features(
    ...
    tokens = [
        identity_model.normalized_tokens(
            str(cluster["canonicalLabel"])
        )
    ...
    word_matrix, char_matrix, text_report = (
        identity_model.build_text_features(clusters, normalized_text)
    )
    builder = identity_model.PairFeatures(
        base_centroids,
        context_centroids,
    ...
    for start in range(0, len(candidate_pairs), batch_size):
        ...
        features[start:stop] = builder.matrix(
...
def candidate_rows(
    ...
    for row_index, (left_index, right_index) in enumerate(pairs):
        ...
        for feature_index, feature_name in enumerate(
            identity_model.FEATURE_NAMES
        ):
            row[feature_name] = float(
                features[row_index, feature_index]
            )
        rows.append(row)
    rows.sort(
        key=lambda row: (
...
def select_nonoverlapping_review(
    ...
    if len(selected) != size:
        raise RuntimeError(
            f"Could select only {len(selected)} disjoint review pairs"
        )
    return selected
...
def write_review_markdown(
    ...
    for ordinal, row in enumerate(selected, start=1):
        ...
        lines.extend(
            [
                f"## {ordinal:03d}. {left['canonicalLabel']} ↔ "
                f"{right['canonicalLabel']}",
                "",
                f"- Type: `{row['pairType']}`",
                f"- Model probability: "
                f"`{float(row['modelProbability']):.6f}`",
                f"- Fused centroid cosine: "
                f"`{float(row['geminiFusedCosine']):.6f}`",
                f"- Tag centroid cosine: "
                f"`{float(row['geminiTagCosine']):.6f}`",
                f"- Context centroid cosine: "
                f"`{float(row['geminiContextCosine']):.6f}`",
                f"- Candidate sources: "
                f"`{', '.join(row['candidateSources'])}`",
                f"- Left `{left['clusterId']}` tags: "
                + " | ".join(left["memberTags"]),
                f"- Right `{right['clusterId']}` tags: "
                + " | ".join(right["memberTags"]),
                "- Review: `PENDING`",
                "",
            ]
        )
...
def main() -> None:
    ...
    if len(clusters) != 19002:
        raise RuntimeError(
            f"Expected 19,002 corrected clusters, found {len(clusters)}"
        )
        ...
        corrected_assignments.append(
            {
                **row,
                ...
                "documented_frozen_merge_applied": (
                    row["final_cluster_id"] == "N-0397"
                ),
            }
        )
    write_csv(
    ...
    pairs, sources = build_cluster_candidates(
        clusters, tag_to_cluster, fused_centroids
    )
    ...
    rows = candidate_rows(
        clusters, pairs, sources, feature_matrix, probabilities
    )
    ...
    write_json(
        OUTPUT / "pass_01_tentative_review_pairs.json", selected
    )
    write_review_markdown(
        OUTPUT / "PASS_01_MANUAL_REVIEW.md", selected, clusters
    )
