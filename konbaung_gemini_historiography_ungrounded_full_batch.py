...
from konbaung_gemini_historiography_ungrounded_triple_trial import (
    DATA_ROOT,
    MODEL,
    TrialResult,
...
DEFAULT_OUTPUT_ROOT = (
    ROOT / "konbaung_historiography_ungrounded_v3_full_batch_20260723"
)
TERMINAL_STATES = {
    "JOB_STATE_SUCCEEDED",
    "JOB_STATE_FAILED",
...
def submit_batch(
    ...
    requests = [
        build_request(job, cache_name, level, max_output_tokens) for job in jobs
    ]
    batch = client.batches.create(
        model=MODEL,
        src=requests,
        config=types.CreateBatchJobConfig(
            display_name=f"konbaung_v3_{label}_{utc_stamp()}"
        ),
    )
...
def wait_for_batch(
    ...
    while True:
        ...
        print(
            json.dumps(
                {"label": label, "state": state, "elapsed_seconds": elapsed}
            ),
            flush=True,
        )
...
def classify_response(
    ...
    try:
        ...
        base.update(
            {
                "category": "truncated"
                if max_tokens_finish or near_cap
                else "schema_invalid",
...
def process_batch(
    ...
    for inlined in responses:
        ...
        if getattr(inlined, "error", None) is not None:
            ...
            attempt = classify_response(
                job, inlined.response, max_output_tokens
            )
        attempt.update(
            {
                "batch_label": label,
...
def retry_transport_errors(
    ...
    retry_jobs = jobs_for_categories(
        jobs_by_key, [source_summary], {"transport_error"}
    )
    if not retry_jobs:
        return None
...
def finalize(
    ...
    triple_rows.sort(
        key=lambda row: (row["volume"], row["page"], row["sid"], row["ordinal"])
    )
    write_text(
    ...
    estimated_batch_usd = (
        noncached_tokens * 0.125
        + cached_tokens * 0.0125
        + output_tokens * 0.75
    ) / 1_000_000
    ...
    report = {
        ...
        "estimated_batch_api_cost_usd_excluding_cache_storage": round(
            estimated_batch_usd, 6
        ),
        "cache": cache_record,
        "existing_databases_modified": False,
    }
...
def run(args: argparse.Namespace) -> None:
    ...
    for volume in (1, 2, 3):
        volume_jobs = [
            job
            for job in jobs
            if job.volume == volume and job.key not in already_accepted
        ]
    ...
    high_truncated = [
        job for job in high_truncated if not accepted_path(out_dir, job).exists()
    ]
    write_json(
    ...
    medium_truncated = jobs_for_categories(
        ...
        [
            summary
            for summary in summaries
            if summary.get("thinking_level") == "medium"
        ],
        {"truncated"},
    )
    medium_truncated = [
        job for job in medium_truncated if not accepted_path(out_dir, job).exists()
    ]
    write_json(
