...
GLOSS_CORRECTIONS = ROOT / "konbaung_predicate_gloss_repair_full_batch_20260718" / "predicate_gloss_corrections.jsonl"
PAGE0208_REPAIR = ROOT / "konbaung_relation_predicate_grounding_page0208_repair_20260718" / "predicate_groundings.jsonl"
...
def load_original_groundings() -> tuple[dict[str, dict[str, str]], dict[str, dict[str, Any]], Counter, list[dict[str, Any]]]:
        ...
        for item in stored.get("response", {}).get("R", []):
            ...
            if recovered:
                ...
                if page_match:
                    ...
                    source = "inferred"
                    status = "unresolved"
                    unresolved.append({"id": item_id, "my": item["my"], "page_id": payload["page_id"], "sid": sid})
...
def main() -> None:
    ...
    for item in read_jsonl(PAGE0208_REPAIR):
        ...
        contexts[item["id"]] = {"sid": item["id"].rsplit("_t", 1)[0], "page_id": "vol1-p0208", "alignment": "sentence_exact_rerun"}
    ...
    for volume in (1, 2, 3):
        write_jsonl(V3 / "triples" / "annotations" / f"vol{volume}.jsonl", [page for page in pages if int(page["volume"]) == volume])
    output_groundings.sort(key=lambda item: item["id"])
    write_jsonl(V3 / "triples" / "predicate_groundings" / "all.jsonl", output_groundings)
    write_json(V3 / "diagnostics" / "predicate_alignment.json", {"counts": dict(alignment_counts), "unresolved": unresolved})
