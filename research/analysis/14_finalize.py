...
class Links(HTMLParser):
 ...
 def handle_starttag(self,tag,attrs):
  ...
  if tag=='img':self.links.append(a['src'])
  if 'id' in a:self.ids.append(a['id'])
q=Links();q.feed((O/'index.html').read_text(encoding='utf-8'))
r=Links();r.feed((O/'source_passages.html').read_text(encoding='utf-8'))
for link in q.links+r.links:
 if '://' in link:continue
 name,_,anchor=link.partition('#');assert (O/name).exists(),link
 if name=='source_passages.html' and anchor:assert anchor in r.ids,link
...
for folder in ['scripts','results','figures']:
 for f in sorted((O/folder).glob('*')):
  if f.is_file():checks.append(hashlib.sha256(f.read_bytes()).hexdigest()+'  '+f.relative_to(O).as_posix())
for pattern in ['*.md','*.html','*.ps1']:
 for f in sorted(O.glob(pattern)):checks.append(hashlib.sha256(f.read_bytes()).hexdigest()+'  '+f.name)
