...
def doc_text(raw):
    with zipfile.ZipFile(io.BytesIO(raw)) as z:
        ...
        for name in z.namelist():
            if name == 'word/document.xml' or re.match(r'word/(footnotes|endnotes|comments|header\d+|footer\d+)\.xml$', name):
...
def consume(source, size, getbytes, depth=0):
    ...
    rec = {'source': source, 'size': size, 'extension': ext, 'text_file': '', 'sha256': '', 'preview': ''}
    records.append(rec)
    try:
        if ext == '.zip' and depth < 5:
            with zipfile.ZipFile(io.BytesIO(getbytes())) as z:
                for info in z.infolist():
                    if not info.is_dir():
                        consume(source + '!/' + info.filename, info.file_size, lambda n=info.filename: z.read(n), depth+1)
        elif ext in {'.docx', '.txt', '.md', '.py', '.r', '.ipynb', '.json', '.yaml', '.yml', '.html', '.csv', '.tsv'}:
            raw = getbytes()
            ...
            if ext == '.docx':
                ...
                for i, row in enumerate(reader):
                    if i < 3: rows.append(row)
                rec['rows'] = i if txt else 0
                rec['columns'] = json.dumps(rows[0] if rows else [], ensure_ascii=False)
                txt = json.dumps({'rows': rec.get('rows'), 'sample': rows}, ensure_ascii=False, indent=2)
            else:
                txt = raw.decode('utf-8-sig', errors='replace')
...
print(json.dumps({'items':len(records), 'extensions':dict(collections.Counter(r['extension'] for r in records)), 'unique_texts':len(list(TEXT.glob('*.txt'))), 'errors':errors}, indent=2))
print('\nDATA / EMBEDDING / CODE CANDIDATES')
for r in records:
    if re.search(r'embedd|\.np[yz]$|\.parquet$|\.pkl$|\.py$|\.r$|triple.*\.(json|csv)$|axial|mapping|cluster', r['source'], re.I):
