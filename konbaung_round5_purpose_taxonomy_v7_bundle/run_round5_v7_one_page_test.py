#!/usr/bin/env python3
"""Run the Round 5 V7 purpose taxonomy on one canonical Konbaung page."""
...
OUT_DIR = ROOT / os.getenv(
    "KONBAUNG_OUT_DIR",
    "konbaung_round5_purpose_taxonomy_v7_one_page_test_20260802",
)
...
TAXONOMY_PROMPT_PATH = BUNDLE_DIR / "konbaung_power_purpose_taxonomy_v7.md"
CATEGORY_SCHEMA_PATH = BUNDLE_DIR / "konbaung_power_purpose_taxonomy_v7.json"
SUBMITTED_PROMPT_NAME = "konbaung_round5_purpose_taxonomy_v7_full_prompt.md"
GENERATION_SCHEMA_NAME = "konbaung_round5_v7_response_schema_simple.json"
VALIDATION_SCHEMA_NAME = "konbaung_round5_v7_response_schema_strict.json"
SOURCE_PAGES_PATH = (
    ROOT
...
def build_submission_prompt() -> str:
    ...
    return (
        "# Konbaung Round 5 political power-purpose classifier, V7\n\n"
        "<ARCHIVE_CONTROL>\n"
        ...
        "No axial entity/relation category codebook is included. The ACTIVE ROUND 5 V7 "
        ...
        + v2
        + "\n</READ_ONLY_V2_AXIAL_CODING_PROMPT_NO_CODEBOOK>\n\n"
        "<ACTIVE_ROUND5_V7_CLASSIFICATION_TASK>\n"
        "For every supplied triple, assign exactly one category ID from the closed V7 "
        "political power-purpose taxonomy below. Classify the individual subject-predicate-object "
...
def render_review(
    ...
    lines = [
        "# Konbaung Round 5 V7 political power-purpose annotation review",
        "",
        f"- Page: `{payload['page_id']}`",
        f"- Model: `{MODEL}`",
        f"- Page selection: {SELECTION_METHOD}",
        f"- Submitted and packaged prompt: `{SUBMITTED_PROMPT_NAME}`",
        "- Prompt composition: original open-coding V1 → V2 axial-coding task without its codebook → ACTIVE Round 5 V7 purpose taxonomy",
    ...
    for sentence in payload["sentences"]:
        ...
        for triple in sentence["triples"]:
            ...
            annotation = f"{category_id} — {category_labels[category_id]}"
            spo = (
...
def main() -> None:
    ...
    write_json(
        OUT_DIR / "run_manifest.json",
        {
            ...
            "prompt_composition": "V1 open-coding research frame; V2 axial-coding task without codebook; ACTIVE Round 5 V7 purpose taxonomy",
