...
from pipeline.translation.evidence_translation_batch import (
    TERMINAL_STATES,
    TranslationJob,
    TranslationResult,
    json_safe,
...
from pipeline.translation.sentence_translation import (
    TRANSLATION_PROMPT,
    normalize,
    response_schema,
    validate,
...
def save_plan(out_dir: Path, label: str, jobs: list[TranslationJob]) -> None:
    write_json(
        out_dir / "batch_plans" / f"{label}.json",
        {
            ...
            "page_count": len(jobs),
            "sentence_count": sum(
                len(build_payload(job.volume, job.page)["sentences"]) for job in jobs
            ),
