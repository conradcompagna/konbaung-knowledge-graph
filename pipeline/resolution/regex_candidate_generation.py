...
TRANSLATIONS_PATH = DATASET / "sentence_translations" / "translations" / "all_volumes.jsonl"
...
def keys_for(value: str) -> dict[str, str]:
    ...
    result["singular_token_order_exact"] = " ".join(sorted(singular(token) for token in row))
...
def main() -> None:
    ...
    records_by_tag = {row["tag"]: row for row in jsonl(NODE_RECORDS_PATH)}
    translations = {row["id"]: row["en"] for row in jsonl(TRANSLATIONS_PATH)}
    ...
    for cluster_index, cluster in enumerate(clusters):
        ...
        for label in labels:
            ...
            if 6 <= len(compact) <= 60:
                numeric_signature = ",".join(token for token in tokens(label) if token.isdigit())
                edit_keys = {compact}
                edit_keys.update(
                    compact[:position] + compact[position + 1 :] for position in range(len(compact))
                )
                ...
                pair_evidence[(left, right)].append(
                    {
                        "rule": rule,
                        "normalizedKey": key,
                        "leftWitnesses": sorted(witnesses[(rule, key, left)]),
                        "rightWitnesses": sorted(witnesses[(rule, key, right)]),
                    }
                )
        ...
        candidates.append(
            {
                ...
                "leftContexts": candidate_context(left_cluster, records_by_tag, translations),
                "rightContexts": candidate_context(right_cluster, records_by_tag, translations),
            }
        )
    candidates.sort(
        key=lambda row: (
