    ...
    for occurrence in jsonl(OCCURRENCES):
        ...
        if len(keys) == 1:
            ...
            triple_vector = np.mean(
                [np.asarray(vector_store[key_to_row[key]], dtype=np.float32) for key in keys],
                axis=0,
                dtype=np.float32,
            )
...
def build_triple_cluster_centroids(tag_vectors: np.ndarray, clusters: list[dict]) -> np.ndarray:
    ...
    for index, cluster in enumerate(clusters):
        ...
        centroids[index] = np.mean(tag_vectors[members], axis=0, dtype=np.float32)
...
def exact_neighbors(
    ...
    with torch.inference_mode():
        for start in range(0, len(weighted_vectors), batch_size):
            ...
            values, found = torch.topk(similarity, k=neighbors, largest=True, sorted=True, dim=1)
...
def unique_ranked_pairs(indices: np.ndarray, scores: np.ndarray) -> list[tuple[float, int, int]]:
...
def main() -> None:
    ...
    neighbor_indices, neighbor_scores = exact_neighbors(weighted_vectors, NEIGHBORS)
    np.savez_compressed(
    ...
    with (
        ...
        for score, left, right in ranked_pairs:
            ...
            markdown.write(f"- **{score:.6%}** — {left_identity} ↔ {right_identity}\n")
