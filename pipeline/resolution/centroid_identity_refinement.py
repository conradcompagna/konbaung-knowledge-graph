...
def centroids_for(clusters: list[dict[str, Any]], tag_vectors: np.ndarray) -> np.ndarray:
...
def exact_neighbors(
    ...
    with torch.inference_mode():
        for start in range(0, len(centroids), batch_size):
            ...
            values, found = torch.topk(similarities, k=neighbors, dim=1, largest=True, sorted=True)
...
def make_batch() -> Path:
    ...
    for (left, right), score in sorted(ranked.items(), key=lambda item: (-item[1], item[0])):
...
def merge_clusters(left: dict[str, Any], right: dict[str, Any]) -> dict[str, Any]:
    survivor, retired = sorted([left, right], key=lambda row: numeric_cluster_id(row["clusterId"]))
    ...
    return {
        ...
        "documentedFrozenMerge": bool(
            left.get("documentedFrozenMerge") or right.get("documentedFrozenMerge")
        ),
...
def apply_adjudication(path: Path) -> None:
    ...
    next_clusters = [row for row in clusters if row["clusterId"] not in merged_ids] + replacements
...
def export_outputs(state: dict[str, Any]) -> None:
    ...
    for cluster in clusters:
        for member_index, member_tag in zip(cluster["memberIndices"], cluster["memberTags"]):
...
def finalize_outputs() -> None:
    ...
    records = read_jsonl(TAG_RECORDS)
    member_indices = [
        member_index for cluster in clusters for member_index in cluster["memberIndices"]
    ]
    ...
    if len(final_pass_rows) != 100 or any(row["decision"] != "REJECT" for row in final_pass_rows):
...
def lexical_pair_score(left: str, right: str) -> dict[str, Any]:
    ...
    lexical = 0.45 * character + 0.30 * tokens + 0.15 * trigrams + 0.10 * token_order
...
def best_cluster_lexical_score(left: dict[str, Any], right: dict[str, Any]) -> dict[str, Any]:
    return max(
        (
...
def write_manual_review_files(start_rank: int = 1, count: int = 1000, rerank: bool = False) -> Path:
    state = json.loads(STATE_PATH.read_text(encoding="utf-8"))
    clusters = state["clusters"]
    records = read_jsonl(TAG_RECORDS)
    ...
    if start_rank == 1 and not rerank:
        ...
        for cluster in clusters:
            ...
            for member_index, member_tag in zip(cluster["memberIndices"], cluster["memberTags"]):
                tagset_lines.append(
                    f"- {markdown_text(member_tag)} | {int(records[member_index]['frequency']):,}"
                )
            tagset_lines.append("")
    ...
    with torch.inference_mode():
        for start in range(0, len(clusters), 256):
            ...
            similarities.masked_fill_(column_indices.unsqueeze(0) <= row_indices.unsqueeze(1), -2.0)
            ...
            for score, flat_index in zip(values.cpu().tolist(), flat_indices.cpu().tolist()):
                if score <= -1.5:
                    continue
    ...
    if rerank:
        reranked = []
        for embedding_rank, (score, left, right) in enumerate(selected, start=start_rank):
        ...
        reranked.sort(key=lambda row: (-row[0], -row[1], row[2], row[3], row[4]))
        candidate_rows = [
            (reranked_rank, *row) for reranked_rank, row in enumerate(reranked, start=1)
        ]
        candidate_lines = [
        ...
        candidate_rows = [
            ...
            for rank, (score, left, right) in enumerate(selected, start=start_rank)
        ]
        candidate_lines = [
    ...
    if rerank:
        candidate_path = OUTPUT / f"RERANKED_ENTITY_MERGE_CANDIDATES_{start_rank}_{end_rank}.md"
    elif start_rank == 1 and count == 1000:
        candidate_path = OUTPUT / "TOP_1000_REMAINING_ENTITY_MERGE_CANDIDATES.md"
    else:
        candidate_path = OUTPUT / f"REMAINING_ENTITY_MERGE_CANDIDATES_{start_rank}_{end_rank}.md"
...
def write_reranked_embedding_pool(
    ...
    centroids = centroids_for(clusters, np.load(TAG_VECTORS, mmap_mode="r"))
    if not torch.cuda.is_available():
        raise RuntimeError("CUDA is required for exact centroid cosine search")
    device = torch.device("cuda")
    corpus = torch.from_numpy(np.ascontiguousarray(centroids)).to(device)
    ...
    with torch.inference_mode():
        for start in range(0, len(clusters), 256):
            ...
            if len(combined_scores) > pool_count:
                keep = np.argpartition(combined_scores, -pool_count)[-pool_count:]
    ...
    for index in range(pool_count):
        ...
        features = best_cluster_lexical_score(clusters[left], clusters[right])
        left_alias_position = clusters[left]["memberTags"].index(features["leftAlias"])
        right_alias_position = clusters[right]["memberTags"].index(features["rightAlias"])
        left_alias_index = clusters[left]["memberIndices"][left_alias_position]
        right_alias_index = clusters[right]["memberIndices"][right_alias_position]
    ...
    for file_start in range(0, pool_count, file_size):
        ...
        output_path = OUTPUT / (f"RERANKED_GEMINI_TOP_{pool_count}_{first_rank}_{last_rank}.md")
...
def cheap_lexical_components(left: dict[str, Any], right: dict[str, Any]) -> tuple[float, float]:
    left_tokens = left["tokens"]
    right_tokens = right["tokens"]
    if left_tokens and right_tokens:
        overlap = sum((left["tokenCounter"] & right["tokenCounter"]).values())
        ...
        for right_index in range(left_index + 1, len(clusters)):
            ...
            for left_alias_index in left_cluster["memberIndices"]:
                ...
                for right_alias_index in clusters[right_index]["memberIndices"]:
                    ...
                    tokens, trigrams = cheap_lexical_components(left_form, right_form)
                    if left_form["compact"] and left_form["compact"] == right_form["compact"]:
                        exact_row = (
                            left_alias_index,
                            right_alias_index,
                            tokens,
                    ...
                    maximum_lexical_upper = max(maximum_lexical_upper, lexical_upper)
                if exact_row is not None:
                    break

            combined_upper = 0.50 * maximum_lexical_upper + 0.50 * embedding
            ...
            if exact_row is not None:
                ...
                for (
                    ...
                    sequence_scored += 1
                    lexical = (
                        0.45 * character + 0.30 * tokens + 0.15 * trigrams + 0.10 * token_order
                    )
...
def write_exhaustive_reranked_candidates(top_count: int = 100_000) -> Path:
    ...
    with ProcessPoolExecutor(max_workers=8) as executor:
        ...
        for done_number, future in enumerate(as_completed(tasks), start=1):
            ...
            print(
                f"Completed {done_number}/{len(tasks)} row chunks ({result['compared']:,} pairs)",
                flush=True,
            )
    ...
    output_path = OUTPUT / "EXHAUSTIVE_RERANKED_TOP_100000_ENTITY_MERGE_CANDIDATES.md"
    output_path.write_text("\n".join(lines), encoding="utf-8")
    if work_dir.resolve().parent != OUTPUT.resolve():
...
def main() -> None:
    ...
    if args.command == "init":
        ...
        for path in write_reranked_embedding_pool(args.pool_count, args.file_size):
            print(path)
    else:
