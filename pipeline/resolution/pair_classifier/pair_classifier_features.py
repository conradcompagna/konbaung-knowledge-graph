...
DIRECTION_AXES = (
    ...
    (
        frozenset(("north", "northern", "northeast", "northwest")),
        frozenset(("south", "southern", "southeast", "southwest")),
    ),
    (
        frozenset(("east", "eastern", "northeast", "southeast")),
        frozenset(("west", "western", "northwest", "southwest")),
    ),
...
KINSHIP = frozenset(
    "son daughter sister brother niece nephew mother father wife husband queen consort grandfather grandmother uncle aunt".split()
)
...
INSTITUTION_MARKERS = frozenset(
    "court hluttaw government authorities monarchy dynasty state sangha".split()
)
...
FEATURE_NAMES = [
    ...
    "extra_generic_fraction",
    "exact_norm",
    "exact_compact",
    "exact_token_multiset",
    "exact_variant_compact",
    "exact_variant_multiset",
    "substring",
    "token_subset",
    "first_token_same",
    "last_token_same",
...
def build_corpus_stats(path: str | Path, rare_cutoff: int = 50) -> CorpusStats:
    ...
    return CorpusStats(
        metas, degrees, top_scores, token_df, token_idf, band_counts, pair_count, rare_cutoff
    )
...
def hard_rejection_reason(left: TagMeta, right: TagMeta) -> Optional[str]:
    ...
    right_place_person = (
        bool(right.places) and not right.persons and bool(left.persons) and not left.places
    )
...
def feature_vector(
    ...
    extra_generic = (
        _safe_div(sum(t in GENERIC_TITLES or t in STOPWORDS for t in extras), len(extras))
        if extras
        else 1.0
    )
    ...
    numberword_conflict = int(
        bool(left.number_words and right.number_words and left.number_words != right.number_words)
    )
    ...
    values = [
        ...
        float(
            "and" in set_left
            and "and" in set_right
            and (set_left - {"and"}) == (set_right - {"and"})
        ),
