...
def collect_labeled_ranks(
    candidate_path: Path, adjudications: pd.DataFrame
    ...
    if missing:
        raise ValueError(
            f"Could not locate {len(missing)} adjudicated pairs in the candidate file; first missing indices: {missing[:10]}"
        )
    return found
...
def choose_auto_threshold(
    ...
    if best is None:
        return 1.0, {
            "predicted": int(deterministic.sum()),
            "true_positive": int(y[deterministic].sum()),
        }
...
def main() -> None:
    ...
    parser.add_argument(
        "--adjudications", type=Path, default=HERE / "TOP_100_PER_BAND_COREFERENCE_REVIEW.md"
    )
    ...
    for index, row in adjudications.iterrows():
        ...
        vector = feature_vector(
            row.left_tag, row.right_tag, row.score_percent, rank_left, rank_right, stats
        )
        ...
        deterministic_reason = (
            None if hard_reason else deterministic_merge_reason(left_meta, right_meta)
        )
    ...
    positive_hard_vetoes = adjudications[
        (adjudications.label == 1) & adjudications.hard_veto_reason.notna()
    ]
    deterministic_false = adjudications[
        (adjudications.label == 0) & adjudications.deterministic_merge_reason.notna()
    ]
    if not positive_hard_vetoes.empty:
        raise RuntimeError(
            "Hard vetoes incorrectly reject adjudicated positives. Review these rows:\n"
            + positive_hard_vetoes[["left_tag", "right_tag", "hard_veto_reason"]].to_string(
                index=False
            )
        )
    if not deterministic_false.empty:
        raise RuntimeError(
            ...
            + deterministic_false[
                ["left_tag", "right_tag", "deterministic_merge_reason"]
    ...
    boosted_oof, _ = grouped_oof_predictions(
        boosted, X, y, groups, max(4, args.cv_repeats // 2), weighted_hist=True
    )
    ...
    review_threshold, review_metrics = choose_review_threshold(
        y, logistic_oof, auto_oof, eligible_review, target_recall=0.90
    )
    ...
    adjudications["oof_review_or_auto"] = auto_oof | (
        eligible_review & (logistic_oof >= review_threshold)
    )
    adjudications["oof_error"] = np.where(
        adjudications.oof_auto_merge & (adjudications.label == 0),
        "false_auto_merge",
        np.where(
            (~adjudications.oof_review_or_auto) & (adjudications.label == 1),
            "missed_valid_merge",
            "",
        ),
    )
    ...
    pd.DataFrame(model_comparison_rows).to_csv(
        args.output_dir / "pair_classifier_model_comparison.csv", index=False
    )
    ...
    summary = {
        ...
        "automatic_merge_policy": {
            ...
            "deterministic_precision_in_training": float(y[deterministic_mask].mean())
    ...
    (args.output_dir / "pair_classifier_training_summary.json").write_text(
...
"""
    ...
    print(
        f"Auto threshold: {auto_threshold:.6f}; review threshold: {review_threshold:.6f}",
        flush=True,
    )
