...
def main() -> None:
    ...
    coref_report = json.loads((COREF_ROOT / "coreference_report.json").read_text(encoding="utf-8"))
    ruler_anchors = json.loads(
        (COREF_ROOT / "ruler_identity_anchors.json").read_text(encoding="utf-8")
    )
    occurrences = {row["occurrenceId"]: row for row in jsonl(EMBEDDING_ROOT / "occurrences.jsonl")}

    cluster_by_id = {cluster["aliasClusterId"]: cluster for cluster in alias_clusters}
    identity_by_cluster = {
        cluster_id: identity
            ...
            writer.writerow(
                {
                    ...
                    "minimum_embedding_similarity": cluster["minimumEmbeddingSimilarity"],
            ...
            writer.writerow(
                {
                    ...
                    "best_candidate_identity": row.get("bestCandidateIdentity") or "",
                    "score": row["score"],
                    "margin": row["margin"],
                    "context_similarity": row["contextSimilarity"],
                    "tag_embedding_similarity": row["tagEmbeddingSimilarity"],
        ...
        source_checks.append(
            {
                ...
                "matches": (actual_size == record["sizeBytes"] and actual_hash == record["sha256"]),
            }
        )

    multi_clusters = [cluster for cluster in alias_clusters if cluster["variantCount"] > 1]
    format_only_clusters = [
        cluster
        for cluster in multi_clusters
        if len({normalized_display(variant["tag"]) for variant in cluster["variants"]}) == 1
    ]
    semantic_clusters = [
        cluster for cluster in multi_clusters if cluster not in format_only_clusters
    ]
    resolved_rows = [row for row in coref_rows if row["status"] == "resolved"]
    resolution_counts = Counter(row["resolvedCanonicalTag"] for row in resolved_rows)
    ...
    integrity = {
        ...
        "mentionRows": sum(1 for _ in jsonl(COREF_ROOT / "mention_records.jsonl")),
        "uniqueMentionIds": len(
            {row["mentionId"] for row in jsonl(COREF_ROOT / "mention_records.jsonl")}
        ),
        "coreferenceAttemptRows": len(coref_rows),
        "uniqueCoreferenceMentionIds": len({row["mentionId"] for row in coref_rows}),
        "acceptedCoreferenceMentions": len(resolved_rows),
        "sourceFiles": source_checks,
        "allSourceFilesUnchanged": all(record["matches"] for record in source_checks),
    }
    ...
    for cluster in semantic_clusters:
        variants = "; ".join(
            f"{variant['tag']} ({variant['frequency']})" for variant in cluster["variants"]
        )
    ...
    identity_lines = [
        f"| {identity} | {count} |" for identity, count in resolution_counts.most_common()
    ]
    generic_lines = [f"| {tag} | {count} |" for tag, count in generic_counts.most_common()]
    unresolved_identities = sorted(set(ruler_anchors) - set(resolution_counts))
...
"""
    (RUN_ROOT / "TWO_PASS_GEMINI_ENTITY_RESOLUTION_REPORT.md").write_text(report, encoding="utf-8")
    print(
        json.dumps(
            {
                "aliasCsv": str(alias_csv),
                "coreferenceCsv": str(coref_csv),
                "report": str(RUN_ROOT / "TWO_PASS_GEMINI_ENTITY_RESOLUTION_REPORT.md"),
                "integrity": integrity,
            },
            ensure_ascii=False,
            indent=2,
