...
DEFAULT_OUTPUT = (
    Path(__file__).resolve().parents[2] / "konbaung_entity_resolution_candidates_20260902_v2"
)
...
def load_source_rows(
    ...
    neighbor_indices = np.empty((len(records), EMBEDDING_CANDIDATES), dtype=np.int32)
    for expected_index, (record, neighbor_row) in enumerate(
        ...
        if len(neighbors) != EMBEDDING_CANDIDATES:
            raise RuntimeError(f"Entity {expected_index} has {len(neighbors)} embedding neighbors")
        neighbor_indices[expected_index] = [int(neighbor["index"]) for neighbor in neighbors]
...
def candidate_rows_for_entity(
    ...
    candidate_indices = np.unique(
        np.concatenate((embedding_neighbor_indices, character_neighbor_indices)).astype(
            np.int32, copy=False
        )
    )
    ...
    embedding_scores = np.asarray(fused_vectors[candidate_indices] @ query_vector, dtype=np.float32)
...
def build(archive_path: Path, output_path: Path) -> None:
    ...
    try:
        ...
        with zipfile.ZipFile(archive_path) as archive:
            ...
            temporary_vector_root = tempfile.TemporaryDirectory(prefix="konbaung_entity_vectors_")
            fused_vectors = extract_fused_vectors(archive, Path(temporary_vector_root.name))
            ...
            if any(not form for form in character_forms):
                empty_index = next(index for index, form in enumerate(character_forms) if not form)
                raise RuntimeError(f"Entity {empty_index} has no alphanumeric character form")
            ...
            with (staging_path / "index.csv").open(
                ...
                for batch_start in range(0, len(tags), CHARACTER_BATCH_SIZE):
                    ...
                    np.maximum(score_matrix, token_sorted_score_matrix, out=score_matrix)
                    for local_index, entity_index in enumerate(range(batch_start, batch_end)):
                        scores = score_matrix[local_index]
                        scores[entity_index] = -1.0
                        top_character_indices = np.argpartition(scores, -CHARACTER_CANDIDATES)[
                            -CHARACTER_CANDIDATES:
                        ]
                        top_character_indices = top_character_indices[
                            np.lexsort((top_character_indices, -scores[top_character_indices]))
                        ]
        ...
        if len(csv_files) != len(records):
            raise RuntimeError(f"Output file count mismatch: {len(csv_files)} != {len(records)}")
        output_path.parent.mkdir(parents=True, exist_ok=True)
