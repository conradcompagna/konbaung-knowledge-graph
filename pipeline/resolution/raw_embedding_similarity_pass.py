...
def main() -> None:
    ...
    neighbor_indices, neighbor_scores = exact_cosine_neighbors(centroids, NEIGHBORS_PER_CLUSTER)
    np.savez_compressed(
    ...
    for score, left, right in review:
        ...
        review_lines.append(f"- **{score:.6%}** — {row['leftIdentity']} ↔ {row['rightIdentity']}")
