...
def best_character_overlap_span(
    canonical_text: str, literal: str
    ...
    if not estimated_starts:
        match = difflib.SequenceMatcher(
            None, compact_literal, compact_text, autojunk=False
        ).find_longest_match()
...
def span_record(
    ...
    fallback_anchors = [
        candidates[0] for candidates in (subject_candidates, object_candidates) if candidates
    ]
    ...
    record = {
        ...
        "characterOverlap": overlap_score
        if method == "maximum_character_overlap"
        else 1.0
        if selected
        else 0.0,
    }
...
def select_endpoints_in_evidence(
    ...
    selected_subject = (
        subject_inside[0]
        if subject_inside
        else (subject_candidates[0] if subject_candidates else None)
    )
    selected_object = (
        object_inside[0] if object_inside else (object_candidates[0] if object_candidates else None)
    )
...
def locate_record(root: Path, phase: str, volume_id: str, page_number: int) -> tuple[Path, str]:
    ...
    if len(matches) != 1:
        raise FileNotFoundError(
            f"Expected one {phase} record for {volume_id}/{filename}; found {matches}"
        )
    return matches[0]
...
def build_page(
    ...
    for input_record in triple_inputs:
        ...
        selected_subject, subject_inferred, selected_object, object_inferred = (
            select_endpoints_in_evidence(subject_candidates, object_candidates, evidence_span)
        )
        ...
        record = {
            ...
            "supportedClaimIds": triple.get("c", [])
            if input_record["source"] == "third_pass"
            else [],
            "validation": {
                "status": status,
                "warnings": warnings,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--third-pass-pilot-root", type=Path, default=DEFAULT_THIRD_PASS_PILOT_ROOT)
