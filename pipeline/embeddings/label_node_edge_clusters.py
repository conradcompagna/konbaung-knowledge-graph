...
DEFAULT_INPUT = PROJECT_ROOT / "konbaung_v3_node_edge_clustering_first_pass_20260724"
    ...
    for attempt in range(1, 6):
        try:
            ...
            if returned_ids != expected_ids:
                raise ValueError(f"Expected cluster IDs {expected_ids}, got {returned_ids}")
            for row in rows:
                if not re.fullmatch(r"[A-Z][A-Z0-9_]*", row["candidateMetaTag"]):
                    raise ValueError(f"Invalid meta-tag {row['candidateMetaTag']!r}")
            usage = (
                response.usage_metadata.model_dump(mode="json") if response.usage_metadata else {}
            )
            return rows, usage
...
def label_kind(
    ...
    payloads = [
        compact_examples(cluster, assignments[cluster["clusterId"]]) for cluster in substantive
    ]
...
def top_tag_text(cluster: dict, count: int = 5) -> str:
    return ", ".join(
        f"{item['tag']} ({item['frequency']})" for item in cluster["frequentTags"][:count]
    )
...
def make_report(
    ...
    for kind, clusters in (("Node", node_clusters), ("Edge", edge_clusters)):
        ...
        for cluster in substantive:
            ...
            split_note = (
                f" **Split/cleanup note:** {label['splitNote']}" if label.get("splitNote") else ""
            )
            lines.extend(
                [
                    f"- **{cluster['clusterId']} — "
...
def main() -> None:
    ...
    labels_by_id = {label["clusterId"]: label for label in node_labels + edge_labels}
    for cluster_id, override in MANUAL_OVERRIDES.items():
        labels_by_id[cluster_id].update(override)
    node_labels = [
        labels_by_id[cluster["clusterId"]] for cluster in node_clusters if not cluster["unresolved"]
    ]
    edge_labels = [
        labels_by_id[cluster["clusterId"]] for cluster in edge_clusters if not cluster["unresolved"]
    ]
    ...
    for kind, assignment_path in (
        ...
        with (
            assignment_path.open("r", encoding="utf-8") as source,
            labeled_path.open("w", encoding="utf-8") as target,
        ):
            for line in source:
                if not line.strip():
                    continue
                ...
                row["candidateMetaTag"] = label["candidateMetaTag"] if label else "UNRESOLVED"
    ...
    (output_dir / "FIRST_PASS_CLUSTER_REPORT.md").write_text(report, encoding="utf-8")
    print(
        json.dumps(
            {
                "passed": audit["passed"],
