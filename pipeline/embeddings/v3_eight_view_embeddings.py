...
def view_inputs(
    ...
    def contextual(header: str) -> str:
        return f"{header}\nBurmese context: {sentence_my}\nEnglish translation: {sentence_en}"
...
def count_token_candidates(
    ...
    candidates = {
        key: spec for key, spec in specs.items() if len(spec.text) > TOKEN_PREFLIGHT_CHAR_THRESHOLD
    }
    if not candidates:
        return {}
    client = make_client()
...
def chunk_oversized_spec(
    ...
    chunks = []
    for ordinal, (text, sentence_my, sentence_en, token_count) in enumerate(accepted, start=1):
...
def prepare() -> None:
    ...
    print(f"Built {len(occurrence_rows):,} occurrences and {len(specs):,} unique embedding inputs")
    token_counts = count_token_candidates(specs)
    for key, token_count in token_counts.items():
        specs[key].token_count = token_count

    oversized_keys = [
        key for key, token_count in token_counts.items() if token_count > SAFE_TOKEN_LIMIT
    ]
    ...
    estimated_cost = estimated_tokens / 1_000_000 * BATCH_PRICE_PER_MILLION_TOKENS_USD
    preflight = {
        "createdAt": now_iso(),
        "model": MODEL,
    ...
    if estimated_cost > BUDGET_CEILING_USD:
        raise RuntimeError(
            f"Projected batch cost ${estimated_cost:.2f} exceeds ${BUDGET_CEILING_USD:.2f} ceiling"
        )
...
def validate_smoke_result(
    ...
    validated = successes == expected and not errors and dimensions == {DIMENSIONS}
    record.update(
        {
    ...
    print(f"Smoke validated: {successes} responses, {DIMENSIONS} dimensions, no errors")
...
def ensure_smoke_accepted() -> None:
    ...
    if (
        ...
        and record.get("state") in {"JOB_STATE_PENDING", "JOB_STATE_RUNNING"}
    ):
    ...
    if (
        ...
        record = {
            ...
            "estimatedCostUsd": round(
                shard_info["estimatedTokens"] / 1_000_000 * BATCH_PRICE_PER_MILLION_TOKENS_USD,
                4,
            ),
        }
        atomic_write_json(record_path, record)
...
def collect(shard: str) -> None:
    ...
    for line_number, row in enumerate(iter_jsonl(raw_path), start=1):
        key = row.get("key")
        if row.get("error"):
            errors.append({"line": line_number, "key": key, "error": row["error"]})
            continue
        try:
            values = extract_embedding_values(row)
        except Exception as exc:
            errors.append({"line": line_number, "key": key, "error": str(exc)})
            continue
...
def finalize() -> None:
    ...
    preflight = read_json(OUTPUT_ROOT / "preflight_report.json")
    request_keys = {row["key"] for row in iter_jsonl(OUTPUT_ROOT / "request_index.jsonl")}
    ...
    actual_cost = actual_tokens / 1_000_000 * BATCH_PRICE_PER_MILLION_TOKENS_USD
    final = {
        "completedAt": now_iso(),
        "model": MODEL,
        "dimensions": DIMENSIONS,
...
def orchestrate(poll_seconds: int, max_hours: float) -> None:
    ...
    while True:
        ...
        if smoke_state == "JOB_STATE_SUCCEEDED" and not smoke_record.get("smokeValidated"):
        ...
        for shard in SHARD_ORDER:
            ...
            try:
                ...
                if getattr(exc, "code", None) == 429 or "RESOURCE_EXHAUSTED" in str(exc):
                    print(
                        f"Batch concurrency quota is full; retaining uploaded input for {shard}",
                        flush=True,
                    )
                    break
                raise
        ...
        certified = {
            shard: (
                ...
                and read_json(OUTPUT_ROOT / "vectors" / f"{shard}.manifest.json").get("certified")
            )
            for shard in SHARD_ORDER
        }
        snapshot = {
