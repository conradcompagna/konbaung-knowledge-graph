...
from pipeline.embeddings import v3_eight_view_embeddings as embedding_run


EMBEDDING_ROOT = embedding_run.OUTPUT_ROOT
OUTPUT_ROOT = embedding_run.ROOT / "konbaung_v3_node_edge_clustering_first_pass_20260724"
...
class VectorStore:
    def __init__(self, shard: str) -> None:
        ...
        self.key_to_row = {row["key"]: int(row["row"]) for row in jsonl_rows(keys_path)}
...
def collect_tag_metadata() -> tuple[dict[str, dict[str, Any]], dict[str, dict[str, Any]]]:
    ...
    for occurrence in jsonl_rows(EMBEDDING_ROOT / "occurrences.jsonl"):
        ...
        for tag, base_view, context_view in (
            ...
            if len(item["sampleOccurrences"]) < 5:
                item["sampleOccurrences"].append({"occurrenceId": occurrence_id, "sid": sid})
...
def build_kind_features(
    ...
    if len(metadata) != expected:
        raise RuntimeError(f"Expected {expected:,} {kind} tags, found {len(metadata):,}")

    ordered = sorted(metadata.values(), key=lambda row: (-int(row["frequency"]), row["tag"]))
    count = len(ordered)
...
def build_features() -> None:
    ...
    result = {
        ...
        "sourceManifestSha256": embedding_run.sha256_file(EMBEDDING_ROOT / "source_manifest.json"),
...
def build_knn(kind: str) -> dict[str, Any]:
    ...
    exact_indices = np.empty((vectors.shape[0], EXACT_NEIGHBORS), dtype=np.int32)
    exact_similarities = np.empty((vectors.shape[0], EXACT_NEIGHBORS), dtype=np.float32)
    for start in range(0, vectors.shape[0], 512):
        ...
        for local, row_index in enumerate(range(start, end)):
            ...
            similarities = candidate_vectors @ np.asarray(vectors[row_index], dtype=np.float32)
...
def build_graph(
    ...
    for source in range(indices.shape[0]):
        for rank in range(min(neighbor_count, indices.shape[1])):
            ...
            weight = max(
                1e-6,
                (similarity - minimum_similarity) / max(1e-6, 1.0 - minimum_similarity),
            )
            if graph.has_edge(source, target):
                ...
                graph.add_edge(source, target, weight=weight, similarity=similarity)
...
def communities_to_labels(communities: list[set[int]], record_count: int) -> np.ndarray:
...
def cohesion_summary(vectors: np.ndarray, communities: list[set[int]]) -> dict[str, float]:
    weighted_sum = 0.0
    total = 0
    cluster_means = []
...
def run_resolution_sweep(
    ...
    for resolution in resolutions:
        ...
        aris = [adjusted_rand_score(label_runs[0], labels) for labels in label_runs[1:]]
        ...
        mention_counts = np.array(
            [sum(int(records[index]["frequency"]) for index in group) for group in chosen],
            dtype=np.int64,
        )
        results.append(
            {
                ...
                "stabilityAdjustedRandMean": (float(np.mean(aris)) if aris else 1.0),
...
def cluster_member_rows(
        ...
        assignments.append(
            {
                ...
                "similarityToCentroid": round(float(similarities[local_offset]), 6),
    ...
    cluster = {
        ...
        "mentionCount": sum(int(records[int(index)]["frequency"]) for index in member_indices),
        "meanSimilarityToCentroid": round(float(similarities.mean()), 6),
        "p10SimilarityToCentroid": round(float(np.quantile(similarities, 0.10)), 6),
...
def write_unlabeled_preview(results: dict[str, Any]) -> None:
    ...
    for kind in ("node", "edge"):
        ...
        clusters = embedding_run.read_json(OUTPUT_ROOT / f"{kind}_clusters.json")
        for cluster in clusters:
            if cluster["unresolved"]:
                continue
            central = ", ".join(item["tag"] for item in cluster["centralTags"][:10])
            frequent = ", ".join(
                f"{item['tag']} ({item['frequency']})" for item in cluster["frequentTags"][:8]
            )
            lines.extend(
                [
                    f"### {cluster['clusterId']}",
...
def main() -> None:
    ...
    select_parser.add_argument("--node-min-similarity", type=float, required=True)
    select_parser.add_argument("--edge-min-similarity", type=float, required=True)
