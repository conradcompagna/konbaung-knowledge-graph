#!/usr/bin/env python3
"""Batch runner for second-pass Konbaung evidence expansion.

It sends the complete first-pass prompt plus the new task through an explicit
Gemini cache and supplies page text plus first-pass results per batch request.
"""
...
from konbaung_gemini_cite_sources_annotator import (
    CiteSourcesResult,
...
DEFAULT_OUT_DIR = Path("konbaung_cite_sources_full_batch")
...
def all_page_jobs(source_root: Path, volume_id: str) -> list[PageJob]:
    ...
    return [job for job in jobs if annotation_path(volume, job.page_num).exists()]


def first_volume_pages(source_root: Path, volume_id: str, count: int) -> list[PageJob]:
    return all_page_jobs(source_root, volume_id)[:count]


def dynamic_page_block(job: PageJob) -> str:
    volume = int(job.volume_id.removeprefix("konbaung_vol"))
    prior = load_json(annotation_path(volume, job.page_num))
    triples = [
        {
            "n": index,
            **{
                key: triple.get(key, "")
                for key in ("s", "sg", "st", "p", "o", "og", "ot", "d", "dg", "l", "lg", "q", "qg")
            },
        }
        for index, triple in enumerate(prior.get("T", []), start=1)
    ]
    return (
        "Now perform NEW_SECOND_PASS_TASK using only the structured schema.\n\n"
        "<metadata>\n"
        f"job_id: {job.job_id}\n"
...
def parse_annotation(response: Any) -> CiteSourcesResult:
    parsed = getattr(response, "parsed", None)
    if parsed is not None:
        return parsed if isinstance(parsed, CiteSourcesResult) else CiteSourcesResult.model_validate(parsed)
    text = response_text(response)
    return CiteSourcesResult.model_validate_json(text)
...
def process_batch_outputs(
    *,
    ...
    for inlined in responses:
        ...
        volume = int(job.volume_id.removeprefix("konbaung_vol"))
        prior = load_json(annotation_path(volume, job.page_num))
        warnings = validate_result(annotation, job.raw_text, len(prior.get("T", [])))
        ...
        validation_record = {
            **page_metadata(job),
            "stage": "cite_sources_second_pass_batch",
            "batch_label": label,
        ...
        row.update(
            {
                ...
                "bucket": bucket,
                "existing_evidence_count": len(normalized["existing"]),
                "addition_block_count": len(normalized["additions"]),
                "added_triple_count": sum(len(item["t"]) for item in normalized["additions"]),
                "usage_metadata": usage,
...
def submit_and_process_full(
    *,
    ...
    for volume_id in ("konbaung_vol1", "konbaung_vol2", "konbaung_vol3"):
        jobs = all_page_jobs(source_root, volume_id)
...
def run_smoke_and_full(args: argparse.Namespace) -> None:
    ...
    if args.prepare_only:
        ...
        smoke_jobs = first_volume_pages(source_root, "konbaung_vol1", args.smoke_pages)
        ...
        readiness = {
            ...
            "schema": "existing=n/e; additions=e/t(s,sg,st,p,o,og,ot,d,dg,l,lg,q,qg)",
