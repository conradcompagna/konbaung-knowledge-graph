...
from pipeline.corpus.build_reader_data import span_candidates
from pipeline.extraction.cite_sources_annotator import ENV_FILE, MODEL, ORIGINAL_PROMPT_PATH, PROMPT
...
class CompletionTriple(BaseModel):
    ...
    c: list[int] = Field(
        description="Numbers of the summary claims directly supported by this triple."
    )
    s: str
    sg: str
    st: str
...
class AnchorAssessment(BaseModel):
    id: str
    how: str = Field(
        description="One sentence explaining how this existing triple anchors an interpretive summary claim."
    )
...
class CoverageGap(BaseModel):
    n: int
    gap: str = Field(
        description="One concise remaining evidence gap between the summary and existing triples."
    )
