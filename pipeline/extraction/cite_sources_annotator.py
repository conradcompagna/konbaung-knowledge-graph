...
ANNOTATION_ROOT = (
    ROOT / "konbaung_open_coding_structured_full_batch_20260709" / "postprocessed" / "valid"
)

OUTPUT_ROOT = ROOT / "konbaung_cite_sources_page_test"

ORIGINAL_PROMPT_PATH = ROOT / "prompts/structured_open_coding.txt"
...
class EvidenceForExistingTriple(BaseModel):
    ...
    e: str = Field(
        description="Exact complete Burmese evidence block from the target page, or empty when unsupported."
    )
...
class SupportingAddition(BaseModel):
    ...
    t: list[AddedTriple] = Field(
        description="New triples directly supported by this evidence block."
    )
...
def validate_result(result: CiteSourcesResult, page_text: str, triple_count: int) -> list[str]:
    ...
    for item in result.existing:
        if item.e and not evidence_is_exact(page_text, item.e):
            warnings.append(
                f"existing {item.n}: evidence is not a whitespace-normalized page substring"
            )
    ...
    for addition_index, addition in enumerate(result.additions, start=1):
        ...
        for triple_index, triple in enumerate(addition.t, start=1):
            if compact_whitespace(triple.s) not in normalized_evidence:
                warnings.append(
                    f"addition {addition_index} triple {triple_index}: subject is not in cited evidence"
                )

            if compact_whitespace(triple.o) not in normalized_evidence:
                warnings.append(
                    f"addition {addition_index} triple {triple_index}: object is not in cited evidence"
                )
...
def markdown_review(
    ...
    lines.extend(
        ...
        or ["- All evidence blocks resolved to the page text."]
    )
