...
from pipeline.extraction.xlmr_seed_annotator import (
    line_start_offsets,
    page_job_from_path,
    write_json,
    write_text,
...
DEFAULT_BASE_PROMPT_FILE = Path("prompts/open_coding_original_prompt.md")
...
class OpenCodedTriple(BaseModel):
    ...
    d: str = Field(
        default="", description="Optional literal Burmese date/time span from TARGET_PAGE_TEXT."
    )
    dg: str = Field(default="", description="English gloss of d, or empty string if d is absent.")
    l: str = Field(
        default="", description="Optional literal Burmese location span from TARGET_PAGE_TEXT."
    )
    lg: str = Field(default="", description="English gloss of l, or empty string if l is absent.")
    q: str = Field(
        default="", description="Optional literal Burmese quantity span from TARGET_PAGE_TEXT."
    )
...
class OpenCodingAnnotation(BaseModel):
    summary: str = Field(
        description="About 250 words summarizing the target page content in English."
    )
...
def structuredize_base_prompt(base_prompt: str) -> str:
    ...
    try:
        ...
        raise ValueError(
            "Base open-coding prompt does not contain the expected section markers"
        ) from exc
...
def resolve_span_whitespace_insensitive(
    raw_text: str, tx: str
    ...
    if boundary_match:
        ...
        if start == -1:
            return (
                None,
                f"boundary start not found after ignoring spaces/newlines: {start_fragment!r}",
            )
...
def validate_annotation(
    job: PageJob, annotation: OpenCodingAnnotation
...
def parse_args(argv: Optional[list[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run structured open coding on one Konbaung OCR page."
    )
    ...
    parser.add_argument(
        "--context-ratio", type=float, default=0.0, help="Ignored; prompts are one page only."
    )
