...
class PropertyKV(BaseModel):
    key: str = Field(
        description="Free-form property key, e.g. title, rank, date, quantity, role, uncertainty."
    )
...
class EntityMention(BaseModel):
    ...
    normalized_name: str = Field(
        description="Best normalized/canonical name for this mention within the chunk."
    )
    entity_type: EntityType
    evidence_quote: str = Field(
        description="Short quote from target text supporting this entity mention."
    )
    description: str = Field(
        description="One-sentence description grounded only in this chunk/context."
    )
...
def looks_like_footnote_line(line: str) -> bool:
    ...
    if re.match(rf"^[{MYANMAR_DIGITS}]+[။.)]", s):
        ...
        if any(
            m in s
            for m in ("ဇန်", "ဖေ", "မတ်", "ဧပြီ", "မေ", "ဇွန်", "ဇူ", "ဩ", "သြ", "စက်", "အောက်", "နို", "ဒီ")
        ):
            return True
...
def clean_page_lines(raw_text: str, page_num: int) -> str:
    ...
    for raw_line in raw_text.splitlines():
        ...
        if (
            line.count("။") >= 2
            and re.search(r"[၁၂၃၄၅၆၇၈၉]\s*။", line)
...
def classify_page(
    page: Page, *, drop_front_matter: bool, drop_appendix: bool, body_start: int
) -> Optional[str]:
...
def parse_volume(
    path: Path, *, drop_front_matter: bool, drop_appendix: bool, body_starts: dict[str, int]
) -> list[Page]:
    ...
    for i, m in enumerate(matches):
        ...
        page.drop_reason = classify_page(
            page,
...
def collect_context(
    segments: list[tuple[str, int]], start: int, end: int, context_words: int, direction: str
) -> str:
    if context_words <= 0:
        return ""
    out: list[str] = []
...
def prepare_chunks(args: argparse.Namespace) -> list[Chunk]:
    ...
    for input_path_str in args.inputs:
        ...
        pages = parse_volume(
            ...
            body_starts=parse_body_starts(args.body_starts),
        )
...
def summarize_outputs(out_dir: Path) -> None:
    ...
    n_chunks = (
        sum(1 for _ in chunks_path.open("r", encoding="utf-8")) if chunks_path.exists() else 0
    )
    n_annotations = (
        sum(1 for _ in annotations_path.open("r", encoding="utf-8"))
...
def parse_args(argv: Optional[list[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Annotate Konbaung chronicle volumes into KG JSONL using Gemini."
    )
    parser.add_argument(
        "--inputs",
        nargs="+",
        required=True,
        help="Input .txt files with [[BOOK=... PAGE=....]] markers.",
    )
    parser.add_argument("--out-dir", required=True, help="Output directory.")
    parser.add_argument("--model", default="gemini-2.5-flash-lite", help="Gemini model ID.")
    parser.add_argument(
        "--api-key", default=None, help="Optional Gemini API key. Prefer GEMINI_API_KEY env var."
    )
    parser.add_argument("--target-words", type=int, default=500, help="Approx target chunk size.")
    parser.add_argument(
        "--max-words", type=int, default=760, help="Hard-ish max chunk size before fallback split."
    )
    parser.add_argument(
        "--context-words", type=int, default=120, help="Approx words before and after target text."
    )
    parser.add_argument(
        "--drop-front-matter",
        action="store_true",
        default=True,
        help="Drop pages before main body. Default true.",
    )
    parser.add_argument(
        "--keep-front-matter",
        dest="drop_front_matter",
        action="store_false",
        help="Keep prefaces/biographies/front matter after cleaning.",
    )
    parser.add_argument(
        "--body-starts",
        default="konbaung_vol1:41,konbaung_vol2:23,konbaung_vol3:23",
        help="Comma map of volume_id:first_body_page. Used only when --drop-front-matter is active.",
    )
    parser.add_argument(
        "--drop-appendix", action="store_true", help="Drop appendix-like sections if detected."
    )
    parser.add_argument(
        "--prepare-only", action="store_true", help="Only normalize/chunk; do not call Gemini."
    )
    parser.add_argument(
        "--limit",
        type=int,
        default=None,
        help="Only annotate first N chunks, useful for pilot runs.",
    )
    parser.add_argument(
        "--resume",
        action="store_true",
        default=True,
        help="Skip chunks already in annotations.raw.jsonl. Default true.",
    )
    parser.add_argument("--no-resume", dest="resume", action="store_false")
    parser.add_argument(
        "--save-prompts", action="store_true", help="Write each prompt to debug_prompts/."
    )
