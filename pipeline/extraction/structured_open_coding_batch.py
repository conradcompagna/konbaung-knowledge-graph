#!/usr/bin/env python3
"""Batch runner for structured Konbaung open-coding annotations.

This uses the same prompt, Pydantic schema, and span validator as
konbaung_gemini_structured_open_coding_annotator.py, but sends requests through
the Gemini Batch API with an explicit cached prompt prefix.
"""
...
from konbaung_gemini_structured_open_coding_annotator import (
    ...
    page_metadata,
    read_base_prompt_and_final_template,
    usage_dict,
    validate_annotation,
)
...
DEFAULT_BASE_PROMPT_FILE = Path("konbaung_open_coding_user_base_prompt_v2.md")
DEFAULT_OUT_DIR = Path("konbaung_open_coding_structured_full_batch")
...
def all_page_jobs(source_root: Path, volume_id: str) -> list[PageJob]:
    ...
    return [page_job_from_path(path) for path in paths]
...
def dynamic_page_block(job: PageJob) -> str:
    return (
        "Now process TARGET_PAGE_TEXT only. Return summary, T, and coverage_report only through the structured schema.\n\n"
        "<metadata>\n"
        f"job_id: {job.job_id}\n"
...
def parse_annotation(response: Any) -> OpenCodingAnnotation:
    parsed = getattr(response, "parsed", None)
    if parsed is not None:
        return parsed if isinstance(parsed, OpenCodingAnnotation) else OpenCodingAnnotation.model_validate(parsed)
    text = response_text(response)
    return OpenCodingAnnotation.model_validate_json(text)
...
def process_batch_outputs(
    *,
    ...
    for inlined in responses:
        ...
        validation_record = {
            **page_metadata(job),
            "stage": "structured_open_coding_batch",
            "batch_label": label,
...
def run_smoke_and_full(args: argparse.Namespace) -> None:
    ...
    static_prefix, _ = read_base_prompt_and_final_template(Path(args.base_prompt_file))
    ...
    if args.process_existing_full:
        ...
        aggregate = {
            ...
            "schema": "summary, T=s/sg/st/p/o/og/ot/d/dg/l/lg/q/qg, coverage_report",
            "full": full_summaries,
        }
