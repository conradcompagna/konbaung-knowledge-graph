#!/usr/bin/env python3
"""Cached Gemini Batch API runner for final summary-gap annotations."""
...
from konbaung_gemini_cite_sources_annotator import ORIGINAL_PROMPT_PATH, PROMPT
from konbaung_gemini_summary_claim_completion_annotator import (
    CompletionResult,
    READER_DATA,
    THIRD_PASS_PROMPT,
    compact_existing,
...
DEFAULT_OUT_DIR = Path("konbaung_summary_gap_full_batch_20260711")
DEFAULT_MODEL = "gemini-3.1-flash-lite"
DEFAULT_MAX_OUTPUT_TOKENS = 10000
DEFAULT_COVERAGE_FILE = Path("konbaung_annotation_coverage_over_20pct_debris_adjusted.json")
...
def completed_final_pages(manifest_path: Path) -> set[tuple[int, int]]:
    ...
    return {
        ...
        if row.get("prompt_variant") == "claim_anchor_gap_analysis"
    }
...
def dynamic_page_block(job: PageJob) -> str:
    ...
    return (
        "Now perform NEW_THIRD_PASS_TASK using only the structured schema.\n\n"
        "<metadata>\n"
        f"job_id: {job.job_id}\n"
...
def parse_annotation(response: Any) -> CompletionResult:
    parsed = getattr(response, "parsed", None)
    if parsed is not None:
        return parsed if isinstance(parsed, CompletionResult) else CompletionResult.model_validate(parsed)
    text = response_text(response)
    return CompletionResult.model_validate_json(text)
...
def validate_completion(annotation: CompletionResult, page: dict[str, Any]) -> list[str]:
    warnings: list[str] = []
    target = normalize_whitespace(page["canonicalText"])
    existing_ids = [item["id"] for item in compact_existing(page)]
    anchor_ids = [item.id for item in annotation.A]
    if sorted(anchor_ids) != sorted(existing_ids):
        missing = sorted(set(existing_ids) - set(anchor_ids))
        extra = sorted(set(anchor_ids) - set(existing_ids))
        warnings.append(f"anchor id mismatch: missing={missing}, extra={extra}")
    claim_ids = {item.n for item in annotation.C}
    for index, triple in enumerate(annotation.T, start=1):
        ...
        invalid_claims = sorted(set(triple.c) - claim_ids)
        if invalid_claims:
            warnings.append(f"T[{index}]: unknown claim ids {invalid_claims}")
    return warnings
...
def process_batch_outputs(
    *,
    ...
    for inlined in responses:
        ...
        validation_record = {
            **page_metadata(job),
            "stage": "summary_gap_anchor_analysis_batch",
            "batch_label": label,
        ...
        row.update(
            {
                ...
                "bucket": bucket,
                "claim_count": len(normalized["C"]),
                "anchor_assessment_count": len(normalized["A"]),
                "gap_count": len(normalized["G"]),
