...
from pipeline.extraction.incremental_categories import (
    MODEL,
    ...
    for directory in LEGACY_RUNS:
        ...
        with connection:
            ...
            for category in result.relation_categories:
                insert_category(connection, "relation", category.id, category.label, page_key)
...
def validate_delta(
    ...
    if any(mapping.category_ref not in valid_entity_refs for mapping in delta.entity_mappings):
        errors.append("Entity mapping references an unknown category")
    if any(mapping.category_ref not in valid_relation_refs for mapping in delta.relation_mappings):
...
def run(
    ...
    with connection:
        ...
        for entity in entities:
            record_assignment(connection, page_key, "entity", entity, resolved_entities[entity])
    ...
    final_entity_categories = [
        Category(id=row["id"], label=row["label"]) for row in category_rows(connection, "entity")
    ]
    final_relation_categories = [
        Category(id=row["id"], label=row["label"]) for row in category_rows(connection, "relation")
    ]
    canonical = CategorizationResult(
        ...
        relation_mappings=[
            RelationMapping(relation=relation, category_id=resolved_relations[relation])
            for relation in relations
        ],
    )
    previous_snapshot = CategorizationResult(
        entity_categories=[Category(id=row["id"], label=row["label"]) for row in entity_catalog],
        relation_categories=[
...
def combine_reviews() -> Path:
    ...
    for run_record in runs:
        ...
        if not review_path.exists():
            raise FileNotFoundError(f"Missing review for {run_record['page_key']}: {review_path}")
...
def export_codebook_markdown() -> Path:
    ...
    for kind, heading in (
        ...
        for category_record in category_records:
            ...
            lines.extend(
                [
                    f"### `{category_record['category_id']}` — {category_record['label']}",
                    "",
...
def refresh_pass_summary() -> Path:
    ...
    for run_record in run_records:
        ...
        payload = json.loads((source_dir / "result.json").read_text(encoding="utf-8"))
        ...
        rows.append(
            {
                ...
                "new_entities": int(payload["validation"]["newEntityCategoryCount"]),
                "new_relations": int(payload["validation"]["newRelationCategoryCount"]),
    ...
    lines.extend(
        [
            ...
            f"- Stored entity assignments: {category_counts['entity_assignments']}",
            f"- Stored relation assignments: {category_counts['relation_assignments']}",
        ]
    )
    complete_thinking = 0
    complete_total = 0
