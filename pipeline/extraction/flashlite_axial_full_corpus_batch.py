...
def load_jobs() -> list[PageJob]:
    ...
    for key, selected_sentences in grouped.items():
        ...
        source_order = {
            sentence["sid"]: index for index, sentence in enumerate(source["sentences"])
        }
...
def validate_result(job: PageJob, result: Any) -> list[str]:
    ...
    if len(annotations) != len(job.triples):
        errors.append(f"annotation count {len(annotations)} != triple count {len(job.triples)}")
    ...
    for kind, proposals, pattern, ids in (
        ...
        for index, proposal in enumerate(proposals):
            ...
            if set(proposal) != proposal_keys:
                errors.append(f"{kind} proposal {index} keys differ: {sorted(proposal)}")
...
def prepare(
    out_dir: Path, max_output_tokens: int
) -> tuple[list[PageJob], str, str, dict[str, Any]]:
    ...
    manifest = {
        ...
        "removed_duplicate_appearances": canonical_index["totals"]["removedDuplicateAppearances"],
...
def load_batch_jobs(out_dir: Path, all_jobs: list[PageJob], volume: int) -> list[PageJob]:
    page_keys = {
        row["key"] for row in read_json(out_dir / "batch_plans" / f"vol{volume}.json")["pages"]
    }
...
def finalize(
    ...
    for job in jobs:
        ...
        page_rows.append(
            {
                ...
                "triple_count": len(job.triples),
                "status": status,
                "new_entity_categories": (result["new_entity_categories"] if result else None),
                "new_relation_categories": (result["new_relation_categories"] if result else None),
            }
        )
        if result is None:
            continue
        for kind in ("new_entity_categories", "new_relation_categories"):
            for proposal in result[kind]:
                proposal_rows.append({"key": job.key, "kind": kind, **proposal})
    ...
    with combined_path.open("w", encoding="utf-8") as combined:
        ...
        for job in jobs:
            combined.write("\n---\n\n")
            combined.write(
                (page_output_dir(out_dir, job) / "review.md").read_text(encoding="utf-8")
            )
    ...
    report = {
        ...
        "prompt_version": "v2.0",
        "thinking_level": "low",
        "max_output_tokens": read_json(out_dir / "source_manifest.json")["max_output_tokens"],
        ...
        "other_unresolved_pages": [row for row in unresolved if row["category"] != "truncated"],
        "usage_new_batch": dict(usage_totals),
        "explicit_cache": cache_record,
        "cached_prefix_verified_in_usage": usage_totals["cached_content_token_count"] > 0,
...
def collect(args: argparse.Namespace) -> dict[str, Any]:
    ...
    nonterminal = [row for row in statuses if row["state"] not in TERMINAL_STATES]
    if nonterminal:
        return {"status": "not_ready", "batches": statuses}
    failed = [row for row in statuses if row["state"] != "JOB_STATE_SUCCEEDED"]
    if failed:
    ...
    for volume in (1, 2, 3):
        label = f"axial_vol{volume}"
        batch_name = read_json(out_dir / "batch_jobs" / f"{label}_submitted.json")["name"]
...
def main() -> None:
    ...
    if args.command == "prepare":
        ...
        result: Any = {
            ...
            "queued_pages": sum(job.key not in COMPLETED_PAGE_DIRS for job in jobs),
            "queued_triples": sum(
                len(job.triples) for job in jobs if job.key not in COMPLETED_PAGE_DIRS
            ),
            "out_dir": str(out_dir),
        }
