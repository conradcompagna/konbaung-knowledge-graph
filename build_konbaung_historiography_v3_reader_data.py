...
def load_occurrences(
    ...
    with SOURCE_PAGES.open("r", encoding="utf-8") as handle:
        for line_number, line in enumerate(handle, start=1):
            ...
            source_by_sid = {
                sentence["sid"]: sentence for sentence in source_sentences
            }
...
def main() -> None:
    ...
    chosen = {
        sid: choose_occurrence(rows) for sid, rows in occurrences.items()
    }

    duplicate_sentence_ids = {
        sid for sid, rows in occurrences.items() if len(rows) > 1
    }
    extra_duplicate_appearances = sum(
        len(occurrences[sid]) - 1 for sid in duplicate_sentence_ids
    )
    duplicate_different_counts = sum(
        len({len(row["triples"]) for row in occurrences[sid]}) > 1
        ...
        record = {
            **identity,
            ...
            "sourceAppearances": [
                ...
                for row in sorted(
                    occurrences[sid], key=lambda row: (row["page"], row["key"])
                )
            ],
        }
    ...
    for volume_id, page_number, sid in page_sentence_appearances:
        ...
        volume_stats[volume_id]["pageAppearanceTriples"] += len(
            selected["triples"]
        )
    ...
    source_sentence_appearances = sum(
    ...
    totals = {
        ...
        "canonicalTriples": sum(
            len(selected["triples"]) for selected in chosen.values()
        ),
    ...
    index = {
        ...
        "pagesWithUnavailableSentences": {
            volume_id: sorted(pages)
            for volume_id, pages in sorted(pages_with_unavailable.items())
        },
        "volumes": [
            {
                "id": volume["id"],
                **dict(volume_stats[volume["id"]]),
                "pagesWithUnavailableSentences": sorted(
                    pages_with_unavailable[volume["id"]]
                ),
            }
            for volume in base_index["volumes"]
        ],
